...
\commentOC{I suggest to remove the last sentence as it is somewhat redundant with “our analysis raises the question…” (here above); we might want to change the above sentence, instead of adding this one, if we really want to highlight the status-quo, cardinal or continuous settings. Alternatively, we could remove the above sentence and keep this final one. (I have no opinion about which would be best.)}\commentBN{I think we must keep the "status-quo, cardinal or continuous settings", we added it because of a review. Remzi proposed it as only one sentence actually, I changed it because I found it too long : "We certainly hope that this is only the beginning of a discussion that may lead to further progress in the future, by underlining that viewing a compromise through the equal loss principle can be of particular interest in richer informational settings with a status-quo point, cardinal individual preferences or a continuum of alternatives."}

\begin{acknowledgements}
	This paper is a part of the ‘Polarization viewed from a social choice perspective’ (POSOP) research project that is carried on under the RDI program funded by Istanbul Bilgi University. We would like to thank POSOP  for the support. We also thank Jean-François Laslier who provided the inspiration and the basis for this article. Last but not least, we thank the associate editor and three anonymous reviewers for their comments and valuable suggestions. \commentOC{I’d write “the anonymous reviewers” so as to include the ones of the previous submission as well (but I have no strong opinion about this).} \commentBN{Ah you mean the ones from the previous journal, you're probably right, maybe just "we thank the associate editor and anonymous reviewers"?}
