...
\title{Compromising as an equal loss principle%\thanks{Grants or other notes
...
\institute{ Université Paris-Dauphine, Université PSL, CNRS, LAMSADE, 75016 PARIS, FRANCE\\
	Corresponding Author \at
	\email{beatrice.napolitano@dauphine.fr} - ORCID iD: 0000-0002-3887-6206
}
...
We introduce two definitions of compromise. In both of them, we pick a spread measure that determines how equally a given vector of real numbers \commentOC{I suggest to delete “real”, as we deal with integer numbers (but there is no point on insisting on this aspect here).} is distributed and we propose to make a collective choice where voters give up from their ideal points “as equally as possible”. The difference between the two is that one, called \emph{egalitarian compromise}, insists on equality at the expense of Pareto efficiency while the other, called \emph{Paretian compromise}, is constrained to pick among the Pareto efficient alternatives. 
The two concepts are logically incompatible. 
\commentOC{It seems to me that pointing out here succinctly the incompatibility of these concepts (and deducing the negative result for Pareto efficient \acp{SCR}) is not kind to the reader, considering that we do it in much more detail later. I suggest to replace “The two concepts are logically incompatible. As a result,” with: “A simple argument will show that”, or something like that.} 
As a result, Pareto efficient \acp{SCR} cannot ensure egalitarian compromises under any spread measure. We prove that several well-known \acp{SCR} such as Condorcet extensions, scoring rules, $q$-approval fall-back bargaining, all fail to be Paretian compromises under any spread measure. We provide examples for which being a Paretian compromise would necessitate to pick an alternative that is, although Pareto optimal, ranked very low by all voters. Such alternative would never be picked by any of the above-mentioned \acp{SCR}. 
...
\commentOC{Please turn the section labels into hyperlinks so the reader can click them (using \textbackslash{}cref or another suitable means of your choice).}
...
We first consider the BK compromise with threshold $q=n$, $f_n$, which corresponds to the rule also known as \textit{Fallback Bargaining} \citep{Brams2001}.
...
In particular we find that: no Condorcet procedure is ECC or PCC (\Cref{th:condorcet}), no scoring rule is ECC (\Cref{th:srECC}) or PCC except for the antiplurality rule (\Cref{th:AntSatsPCC,th:srPCC}), BK compromises are neither ECC or PCC (\Cref{th:BKthreshold,th:FBfailsECC}) except for Fallback Bargaining that is PC (\Cref{th:FBsatsPC}).
...
We also want to mention that different notions of compromise can be conceived. \citet{Borgers1991} defines as compromises all Pareto-optimal alternatives that are not the top choice of any individual. In this context a compromise does not always exist. This is also a possible approach to adopt and we thank the anonymous reviewer for the valuable remark.
\commentOC{I find this phrasing a bit odd. Also, I find this idea very arbitrary. I suggest: “This is also a possible approach to adopt and we thank an anonymous reviewer for this remark.”}
...
\commentOC{I suggest to remove the last sentence as it is somewhat redundant with “our analysis raises the question…” (here above); we might want to change the above sentence, instead of adding this one, if we really want to highlight the status-quo, cardinal or continuous settings. Alternatively, we could remove the above sentence and keep this final one. (I have no opinion about which would be best.)}

\begin{acknowledgements}
	This paper is a part of the ‘Polarization viewed from a social choice perspective’ (POSOP) research project that is carried on under the RDI program funded by Istanbul Bilgi University. We would like to thank POSOP  for the support. We also thank Jean-François Laslier who provided the inspiration and the basis for this article. Last but not least, we thank the associate editor and three anonymous reviewers for their comments and valuable suggestions. \commentOC{I’d write “the anonymous reviewers” so as to include the ones of the previous submission as well (but I have no strong opinion about this).}
