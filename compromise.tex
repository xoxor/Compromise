...
We first consider the BK compromise with threshold $q=n$, $f_n$, which correspond to the rule known as \textit{Fallback Bargaining} \citep{Brams2001}.
...
It is important to note, as seen in \Cref{sec:RestrictionOnSigma}, that all these impossibility results are independent of the concept of \textit{equity} that might be used.
In fact, these results hold under $\Sigma$, which is the largest set of spread measures one can consider. Any particular notion of equity that one might envision would fall under $\Sigma$, and the impossibilities would still prevail.
The positive results are not propagated however, in fact, as soon as we define a reasonably mild restriction of $\Sigma$ on four or more voters neither Fallback Bargaining nor antiplurality are any longer PCC (\Cref{th:3votRestriction}).

With two individuals and a restriction similar in spirit, we see that all well-known two-person \acp{SCR} of the literature, namely, Fallback Bargaining, Pareto and veto rules, short listing and veto rank, fail to pick ex-post compromises (\Cref{th:2votPCC}).
...
After all, as one anonymous reviewer commented and whom we particularly thank for grasping our objective, the tension between equity and efficiency is not new in economics. In our paper we try to cast this tension in a context where it does not seem to have been considered yet. We certainly hope that this is only the beginning of a discussion that may lead to further progress in the future.
...
We close by noting that viewing a compromise through the equal loss principle can be of particular interest in richer informational settings with a status-quo point, cardinal individual preferences or a continuum of alternatives.


\begin{acknowledgements}
	This paper is a part of 'Polarization viewed from a social choice perspective' (POSOP) research project that is carried on under the RDI program funded by İstanbul Bilgi University. We would like to thank POSOP  for the support. We also thank Jean-François Laslier who provided the inspiration and the basis for this article. Last but not least, we thank all anonymous reviewers for their comments and valuable suggestions.
