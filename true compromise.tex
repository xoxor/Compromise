...
We call antiplurality score vector the score vector $w$ formed such that $w_{i}=1 \forall i\in \intvl{0, m-1}$ and $w_{m}=0$.
...
\begin{proof}
Take $\bar{\sigma }\in \Sigma $ defined for each $(l_{1}, \dots ,l_{n}) \in \intvl{0,m-1}^N$ as $\bar{\sigma}(l_{1},...,l_{n})=1$ if 
...
\begin{definition}
	...
	Give any $m\geq3$ and $n\geq3$, we say that a spread measure $\sigma$ satisfies condition $C_{m,n}$ iff we have $\sigma(m-3, m-1, m-2, \dots, m-2) < \sigma(m-2, m-3, \dots, 1, 0, 0)$.
\end{definition}
