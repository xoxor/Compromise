...
\]
\commentOC{$\lambda^P: A → \alllosses$. Define ${\lambda^P}^{-1}: \lambda^P(A) → A$. Define $<_\sigma$ and $≤_\sigma$ as binary relations over $\alllosses$. Given $L \subseteq \alllosses$, define $\min_{<_\sigma}L = \set{l \in L \suchthat \forall l' \in L: l ≤_\sigma l'}$ as the minimal elements of $L$ considering $<_\sigma$. Then, $\musigma(P) = {\lambda^P}^{-1}(\min_{<_\sigma}(\lambda^P(A))$. Define $<_{\sigma \circ \lambda^P} \subseteq A × A$ and $x <_{\sigma \circ \lambda^P} y \iff \lambda^P(x) <_\sigma \lambda^P(y)$. Then, $\musigma(P) = \min_{<_{\sigma \circ \lambda^P}}(A)$.}
...
\begin{remark}
\commentRS{Remark 3.1 (only parts i and ii) can go right after the proof of Proposition 3.1, noting that it will be later used in the proof of Proposition 3.2. As in Remark 3.1, ii directly implies iii, we can still use it for Proposition 3.2 when showing EC implies not PCC. }
	\Cref{prop:nonParetian} does not hold for $m=2$: in that case, the largest Paretian SCR, $f(P) = \paretopt(P)$, is ECC. In fact, $\forall \sigma, P: \paretopt(P) \cap \musigma(P) ≠ \emptyset$. This is because with only two alternatives, all profiles have this form, with $k < n$:
	\begin{center}
		$
		\begin{array}{ccc}
		\mathbf{k} \quad &a&b\\
...
\commentRS{Section 3.2 must have more of a narrative of what we are doing, with more comments on Proposition 3.2 and Remark 3.2.}
The previous section shows the tension between being Paretian and insisting on selecting egalitarian alternatives, even when they are Pareto dominated. This section investigates the consequences of inverting the order of priorities: the compromise rules defined here insist that the winning alternatives be Pareto optimal, and among those, select the ones associated to most equally distributed loss vectors.
...
It will also probably come with no surprise that being a PC is incompatible with being ECC, and that being an EC is incompatible with being PCC. The first fact is immediate, observing that being a PC requires to be Paretian, which permits to use \cref{prop:nonParetian}. The second is less immediate, because being PCC does not require to be Paretian.
...
\begin{proof}
	...
	Letting $\Ptop$ denote the profile of \cref{prop:muSigmaLast} and considering any EC $f$ and any $\sigma \in \Sigma$, suffices to prove that $f(\Ptop) \cap \mustar(\Ptop) = \emptyset$.
	
	From \cref{prop:muSigmaLast}, $\musigma(\Ptop) \cap \paretopt(\Ptop) = \emptyset$, hence $\musigma(\Ptop) \cap \mustar(\Ptop) = \emptyset$.
	
	Also, because $f$ is an EC, for some $\sigmatop$, $f(\Ptop) \subseteq \mu_{\sigmatop}(\Ptop)$. Using \cref{prop:muSigmaLast} again, we see that $\forall \sigma' \in \Sigma$, $\musigma(\Ptop) = \mu_{\sigma'}(\Ptop)$, and in particular, $\musigma(\Ptop) = \mu_{\sigmatop}(\Ptop)$, hence $f(\Ptop) \subseteq \musigma(\Ptop)$.
	
	From these two facts follow that $f(\Ptop) \cap \mustar(\Ptop) = \emptyset$.
\end{proof}

\begin{remark}
	\commentOC{This used to be Remark 3.1}
...
\begin{remark}
	\commentOC{This used to be Remark 3.2.}
	\commentRS{Remark 3.2 should mention n and m }
