...
We recall from \cref{sec:introduction} the definition of \textit{fallback bargaining (FB)} \citet{Brams2001}, a procedure that, starting from the top choices of each voter, it descends ranks until a unanimity consensus is found. Such alternative is selected as winner. In the context of bargaining we introduce other procedures. \textit{Veto Ranking (VR)} is a commonly used mechanism in the selection of arbitrators \commentBN{add references}. Given a list of $m$ (odd) candidates, each of the two parties vetoes $\frac{m-1}{2}$ alternatives and rank the remaining ones. The selected alternative is the one ranked the best (summing ranks in the two preference orderings) among the ones who have not been vetoed. In case of ties, an alternative is randomly chosen among those. \textit{Shortlisting (SL)} \citet{Clippel2014} is another mechanism where one of the two parties selects $\frac{m+1}{2}$ alternatives (thus vetoing the remaining ones), and then the second party chooses an alternative out of this subset. \textit{Pareto-and-Veto (PV)} procedure \citet{Laslier2020} is a solution that distribute the veto powers among the voters. One of the two parties starts by vetoing $v_1$ alternatives, the other one proceeds then to vetoes $v_2$ alternatives, where $v_1+v_2=m-1$. The remaining candidate is elected.

\begin{theorem}
	Let $m \geq 4$. FB, VR, SL, and PV fail PCC.
\end{theorem}
\begin{proof}
	Without loss of generality let us consider $m=2k+1$ an odd number of alternatives. Consider the following profile P:
		\begin{center}
			$
			\begin{array}{cccccccccc}
				\mathbf{i_1} \quad &x&a_1&\dots&&a_k&y&b_1&\dots&b_{k-1}\\
				\mathbf{i_2} \quad &b_1&\dots&b_{k-1}&y&x&a_1&\dots&&a_k\\
			\end{array}
			$
		\end{center}
	We can note that $\sigma(\lambda_{P}(y)) < \sigma(\lambda_{P}(x))$ $\forall \sigma \in \Sigma^{C_{m,2}}$ for some $k \in \N$. In fact, when $k$ gets incremented by one, an $a_{k+1}$ and a $b_{k+1}$ are added to the profile, thus not changing the spread of $y$. $\sigma(k+1,k-1)<\sigma(0,k)$ which is true for some k.
	With the VR rule, the last $k$ alternatives get vetoed and, among the $k+1$ remaining, $x$ has the highest Borda score. 
	The SL rule picks $x$.
	With PV, for each $v_1$ and $v_2$ there exist a profile such that $y$ is excluded. \commentBN{phrase this better}
\end{proof}
