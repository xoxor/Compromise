...
Consider a finite set $N$ of individuals with $\#N=n\geq 2$ and a finite set $A$ of alternatives with $\#A=m\geq 2$. We write $\linors$ for the set of linear orders (a connected, transitive, asymmetric relation) over the set of alternatives $A$. A social choice rule (SCR) is a mapping $f:\linors^{N}\rightarrow 2^{A} \setminus \{\emptyset \}$. Each individual $i \in N$ is associated to her preference order $\pref_i \in \linors$, which we also denote with $P_i$. Following the classical nomenclature, we call {\em profile} $P=(P_1, \dots, P_n)$ the set of preference orders for each individual. We define the rank (position) of alternative $x$ in the preference order of the individual $i$ as the number of alternatives that $i$ prefers to $x$: $r_i(x)=\#\{a \in A | b\prefeq_i a , b\in A\}$. Given a profile $P$ and two alternatives $x$ and $y$, if every individual ranks $x$ over $y$ than we say that $x$ Pareto dominates $y$ and that $y$ is Pareto dominated. We write $\paretopt(P)$ for the set of Pareto optimal alternatives, i.e., the alternatives which are not Pareto dominated. 
%A SCR $f$ is Pareto optimal if $f(P)$ never contains a Pareto dominated alternative. 

We write with $\lambda^{P}_i(x)=P_{i}(x)-1$ the loss of ranks for $i$ when electing $x$ instead of her favorite alternative. This value can also be seen as the number of pairwise flips we need to perform in order to place $x$ at top position for $i$. Thus, $\lambda ^{P}(x)=(\lambda _{i}^{P}(x))_{i\in N}\in \R_+^N$ represents the vector of losses when electing $x$. 
Considering a loss vector, we are interested in measuring its \oquot spread\cquot. We define a spread measure as a function $\sigma: \R_{+}^{N}\longrightarrow \R_{+}$ that associates a spread value to each possible loss vector.
We call $\Sigma$ the set of spread measures such that for each $\sigma \in \Sigma$ and for each vector $l \in R_{+}^{N}$,  $\sigma(l_1, \dots, l_n)=0 \iff l_i=l_j$ $\forall i,j \in \{1, \dots, n\}$ . When all the values are the same, a coefficient of zero expresses perfect equality.

\section{Egalitarian versus Paretian compromises}
We define the set $\musigma(P)$ as the set of the alternatives whose loss vectors are the most equally distributed according to the spread measure $\sigma$; and $\mustar(P)$ as the set of such alternatives selected among the Pareto optimal ones. 
\begin{align}
...
\end{proposition}
	\begin{proof} 
		...
		\begin{center}
			$
			\begin{array}{ccccc}
			\mathbf{1} \quad &a&b&\dots&z\\
			\mathbf{n-1} \quad &b&a&\dots&z\\
			\end{array}
			$
		\end{center}
		where we have $m$ alternatives disposed such that only one alternative $z$ is ranked at the same position by every voter, and it is ranked last.
		For each $\sigma \in \Sigma$ the spread associated to every alternative except $z$ is greater than zero $\sigma(\lambda^P(x))>0$, $\forall x \in A \ \{z\}$; whilst $\sigma(\lambda^P(z))=0$. Thus, the set $\musigma(P)$ of the \oquot most equal\cquot \ alternatives consists of the only element $z$. On the other hand, the set $\mustar(P)$ will never contain $z$ whatever is the choice of $\sigma$ since it is a Pareto dominated alternative.
	\end{proof}
\begin{note}
...
\begin{definition} A SCR $f$ is an \textit{Egalitarian Compromise} EC iff \[\exists \sigma \in \Sigma \suchthat \forall P \in \linors^N: f(P) \subseteq \musigma(P).\]
\end{definition}

\begin{definition} A SCR $f$ is a \textit{Paretian Compromise} PC iff \[\exists \sigma \in \Sigma \suchthat \forall P \in \linors^N: f(P) \subseteq \mustar(P).\]
\end{definition}

\begin{definition} A SCR $f$ is \textit{Egalitarian Compromise Compatible} ECC iff \[\exists \sigma \in \Sigma \suchthat \forall P \in \linors^N: f(P) \cap \musigma(P) \neq \emptyset.\]
\end{definition}

\begin{definition} A SCR $f$ is \textit{Paretian Compromise Compatible} PCC iff \[\exists \sigma \in \Sigma \suchthat \forall P \in \linors^N: f(P) \cap \mustar(P) \neq \emptyset.\]
\end{definition}
...
In what follows, we analyze some relationships between these classes. 

It follows from the definitions that if a SCR $f$ is an Egalitarian (or a Paretian) Compromise then $f$ is also Egalitarian (or Paretian, respectively) Compromise Compatible: 
...
\end{proposition}
\begin{proof}
%	We already showed, in the proof of Proposition \ref{prop:disjoint}, that for $m=2$ and $\forall \sigma \in \Sigma$ and $\forall P \in \linors^N$ $\musigma(P) \cap \mustar(P) \neq 0$. So this proposition does not hold for $m<3$.
	...
	\begin{center}
		...
		$
	\end{center}
	Consider any SCR $f$ that is an Egalitarian Compromise, and any $\sigma \in \Sigma$. In order to prove the first implication, we will show that $f(P) \cap \mustar(P) = \emptyset$.
	Observe that $z$ is the only alternative contained in the set $\musigma(P)$, so since $f$ is an EC then $f(P)={z}$. But $z$ is Pareto dominated, thus $\mustar(P)$ does not contain $z$. Therefore, $f(P) \cap \mustar(P) = \emptyset$ and the first implication is proved.
	
	Moreover, we can observe that $\musigma(P)={z}$ and $z \notin \mustar(P)$, not only for some particular spread measure, but for all $\sigma \in \Sigma$. This proves the second implication.
\end{proof}
\begin{corollary} \label{cor:optECC}
...
\begin{note}
	...
	In fact, considering Proposition \ref{prop:disjoint}, the existence of a spread measure $\sigma$ such that our SCR $f$ picks some alternatives which belong to the set $\musigma$ is not a guarantee that they are also Pareto optimal, thus belonging to the set $\mustar$ (and vice-versa).
\end{note}
...
\begin{proof}
	...
	In order to prove it for Anti-plurality, consider the profile described in the proof of Proposition \ref{prop:disjoint}, where the most equal alternative $z$ is ranked last by all voters. This alternative is the only element of the set $\musigma$ but it is not selected by anti-plurality.
\end{proof}

\begin{proposition} For $n>2$, $m\geq3$ \commentBN{check for n=2 (classical bargaining setting)} \label{prop:rulesnotPCC}
	\begin{item}
		\item[1)] Condorcet consistent rules are not $PCC$
		\item[2)] BK compromise for $q \in \{1, \dots n-1\}$ are not $PCC$
		\item[3)] Ranked runoff rules (like STV) are not $PCC$
...
\noindent Please note that these impossibility results prevail for any restrictions $D \subseteq \Sigma$.

\begin{proposition}
	For $n>2$, $m=3$ some scoring rules are  $PCC$
\end{proposition}
\begin{proof}
	...
	\begin{center}
		...
		$
	\end{center}
	Let $s:\{1,\dots,m\}\rightarrow [0,1]$ be a scoring function that assigns $1$ to the alternative ranked at the first position, and $0$ to the one ranked at the last. Therefore, the alternative ranked in the second position has a score $0 \leq \lambda<1$. We exclude $\lambda = 1$ which is anti-plurality.
