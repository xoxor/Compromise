...
\begin{definition}
    Given any $m \geq 4$ and $n=2$, we say that a spread measure $\sigma$ satisfies condition $D$ iff we have $\sigma(k-1,k+1)<\sigma(0,k)$ for some $k$.
\end{definition}

\begin{theorem}
	Let $m \geq 4$ and $\sigma \in \Sigma^{C_{m}}$. FB and PV fail PCC. Furthermore if $m$ is odd, also VR and SL fail PCC.
\end{theorem}
\begin{proof}
	Without loss of generality let us consider $m=2k+1$ an odd number of alternatives. Consider the following profile P:
		\begin{center}
			...
			$
		\end{center}
	We can note that $\sigma(\lambda_{P}(y)) < \sigma(\lambda_{P}(x))$ $\forall \sigma \in \Sigma^{C_{m,2}}$ for some $k \in \N$. In fact, when $k$ gets incremented by one, an $a_{k+1}$ and a $b_{k+1}$ are added to the profile, thus not changing the spread of $y$. Therefore any rule satisfying PCC would select $y$ since $\sigma(k+1,k-1)<\sigma(0,k)$ is true for some k.
	...
	For FB and PV we can add an additional alternative at the end to show the case m even.
\end{proof}
