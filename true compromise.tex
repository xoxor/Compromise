...
\begin{proof}
	...
	Let $\min\lprof(x)(N)$ and $\max\lprof(x)(N)$ designate the minimal and maximal value of the loss vector of $x$.
	Define $\rho = \min\set{l \in \intvl{0, m - 1} \suchthat \exists x \suchthat \max\lprof(x)(N) = l}$. 
	Thus, $\nexists x \suchthat \max\lprof(x)(N) < \rho$.
	Recall that, by definition, $\FBP = \set{x \suchthat \max\lprof(x)(N) ≤ \rho}$, or equivalently, $\FBP = \set{x \suchthat \max\lprof(x)(N) = \rho}$.
	
	Pick any $x \in \FBP$. 
	Pick any $i \in N$ such that $\lprofi[x] = \min\lprof(N)$ and define $\ibar$ as the other individual, thus with $\lprofibar[x] = \max\lprof(N) = \rho$. Define $z$ as the alternative such that $\lprofi[z] = \rho$.
	
 	We show four facts that are true for any value of $m$, with the definitions so far and $\sigma(l_1, l_2) = \abs{l_1 - l_2}$, and which together prove the statement. First, $\minineq[\FBP] ≤ \frac{m}{2}$. Second, if $\minineq[\FBP] = \frac{m}{2}$, then $\FBP = \set{x, z}$. Third, if $\FBP = \set{x, z}$, then $\minineq[\FBP] = \minineq[\POP]$. Fourth, if $\minineq[\FBP] ≤ 2$, then $\minineq[\FBP] = \minineq[\POP]$. 

	When $m ≤ 6$, using the first fact, $\minineq[\FBP] ≤ 3$, and suffices then to use either, if $\minineq[\FBP] = 3$, the second and the third facts, or otherwise, the fourth fact, to prove as required that some alternative in $\FBP$ reaches the required minimal inequality over the Pareto optimal alternatives.
 	
 	\emph{First fact}
 	
	Define $S_1 = \set{y ≠ x \suchthat 0 ≤ \lprofi[y] < \rho}$ and $S_2 = \set{y \suchthat \rho < \lprofibar[y] ≤ m - 1}$.
 	By definition of $\rho$, $\nexists y \suchthat \max\lprof(y)(N) < \rho$,
	...
	Also, $\card{S_2} = m - 1 - \rho$. We obtain $m - 1 - \rho ≥ \rho - 1$, thus $\rho ≤ \frac{m}{2}$. And by definition of $\rho$, $\minineq[\FBP] ≤ \rho$.
	
	\emph{Second fact}
	
	Pursuing with $S_1$ and $S_2$ as defined above, and assuming further that $\rho = \frac{m}{2}$, or equivalently $m - \rho = \rho$, we see that $S_1$ and $S_2$ have the same cardinalities, hence, are equal. By definition of $z$, $z \notin S_1$. Thus, $z \notin S_2$. Therefore, $\lprofibar[z] ≤ \rho$, and as $\lprofi[z] = \rho$, $z \in \FBP$. As $\FBP = \set{y \suchthat \max\lprof(y)(N) = \rho}$, $y \in \FBP$ requires that $\lprofi[y] = \rho$ or $\lprofibar[y] = \rho$, thus, no alternative but $x$ and $z$ may be in $\FBP$.
