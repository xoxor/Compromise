...
i\in \intvl{1, m-1}$. A score vector $w$ is \emph{strict} iff $w_{i}>w_{i+1}$ $\forall i\in \intvl{1, m-1}$. Given a score vector $w$, we write $s^{w}(x,P)=\sum_{i\in N}w_{r_{\prefi}(x)}$ for the score of $x\in A$ at $P\in L(A)^{N}$. Every score vector $w$ identifies a \emph{scoring rule} $f^w_n$ defined as $f^w_n(P)=\left\{ x\in
...
\commentBN{Define antiplurality score vector.}

\begin{theorem}
	Let $m\geq 3.$ Take an antiplurality score vector $w$. The SCR $f_{n}^{w}$ satisfies PCC for all $n\geq 3$.
\end{theorem}

\commentBN{Remark that antiplurality fails PC.}
...
\commentBN{adjust the proof}

\begin{proof}
	Take any $m\geq 3$ and any score vector $w.$To show the "if" part, let $w$	be the antiplurality score vector and take any $n\geq 3.$CONTINUE\ WITH THE PROOF IN PROPOSITION \ref{prop:antPCC}.
	...
	Let $n=\max \left\{ n_{1,}n_{2}\right\} $.Let $A=\left\{ a_{1}, \dots, a_{m}\right\} $. Take some $P\in L(A)^{N}$ with
...
\commentBN{Define condition C.}
\commentRS{Let’s say that a spread measure sigma satisfies condition C iff we have $\sigma(m-3, m-1. m-2, ....., m-2) < \sigma(m-2, m-3,..., 1, 0, 0)$ for some value of m and n. As such the condition is technical. We can motivate it through some arguments but more importantly, we should be able to show that several spread measures of the literature (which Beatrice is discovering) satisfy condition C. We can than take the subset of capital sigma that consists of spread measures that satisfy condition C and prove the theorem under this restriction.}
