...
\end{proposition}
\begin{proof}
Given any utility assignments $v$ and $v^{\prime}$, and a spread measure $\sigma^k$, we need to show that it is possible to choose $k^\prime \in \R$ such that $C^{\sigma^k} \circ \vpr = C^{\sigma^{k'}} \circ v^{\prime \text{pr}}$. Let  $t \in \intvl{1, m}$ be the position such that the loss of any alternative $x \in A$ ranked $t$ in $v$ by any of the voters is greater than or equal to $k$. \commentOC{As defined here, $t$ is not well-defined as it is usually not unique (if some $t$ satisfies the definition below, then $t' > t$ also satisfy the definition). Other problem: for $k$ very big, no $t$ satisfy the definition.}
...
\begin{definition}
\label{D1} \bigskip We say that a SCR $f:L(A)^{N}\rightarrow 2^{A} \setminus \{\emptyset \}$ is a $\Sigma$-ordinal compromise iff there exists $\sigma \in \Sigma $ and an UA $v$ such that $C^{\sigma }\circ \vpr=f$. \commentOC{Please check whether we mean $v$ or $\vpr$ here (and below) and correct accordingly.}
\end{definition}
...
\commentOC{The proof does not hold here with $\SPPd$: we can’t use \cref{D2} (as we do one paragraph below). To be checked.} \commentBN{Didn't get which proof you're referring to.}
...
where under any $\sigma \in \Sigma $ \commentBN{$\sigma \in \SPPd$}, $C^{\sigma }\circ v$ picks $y$ for the UA $v(k)=n-k+1$ while FB picks $x$. So FB fails Definition \ref{D2}, hence Definition \ref{D1}.
...
where under any $\sigma \in \Sigma $ \commentBN{$\sigma \in \SPPd$}, $C^{\sigma }\circ v$ picks $d$ for the UA $v(k)=n-k+1$ while anti plurality picks $e$.
