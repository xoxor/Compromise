...
\begin{definition}
	We say that a spread measure $\sigma$ satisfies condition C iff we have $\sigma(m-3, m-1, m-2, \dots, m-2) < \sigma(m-2, m-3, \dots, 1, 0, 0)$, for some $n\geq3$ and for some $m\geq3$. \commentBN{Should this be for all $n$ in order to make our assertions in Theorems ?}
\end{definition}
...
\begin{theorem}
	Let $m\geq 3$ and take the antiplurality score vector $w$. If $\sigma$ satisfies condition C then there exist $n\geq 3$ such that $f_n^{w}$ fails PCC.
\end{theorem}

\begin{theorem}
	Let $m\geq 3$. If $\sigma$ satisfies condition C then there exist $n\geq 3$ such that the BK compromise  $f_n$ fails PC.
\end{theorem}

\commentBN{adapt to Th. 10 and 11}
	\begin{proof}
...
\subsection{Old description of "reasonability"}
...
\begin{theorem}
	Let $m\geq 3.$ When $\Sigma $ is furthermore reasonable, there exist $n\geq 3 $ such that both the antiplurality rule $f_n^{w}$ and $f_{n}$ fail PCC.
	\end{theorem}
