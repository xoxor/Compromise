...
\begin{proof} for $\sigma_{ad}$. \\
	...
	\[s(l)=[|m-3-m+2|+|m-1-m+2|+ 0 + \dots + 0]= \frac{2}{n}\] 

	For $l_2$, the arithmetic mean is 
	\[\bar{l_2}= \frac{1}{n}\sum_{i=2}^{m-1}{m-i}= \frac{(m-2)(m-1)}{2n}\]
	\begin{align}
		s(l_2)&= \sum_{i=2}^{m-1}{|(m-i)-\frac{(m-2)(m-1)}{2n}|+(n-(m-2))|\frac{(m-2)(m-1)}{2n}|} \\
		&= \sum_{i=2}^{m-1}{|(m-i)-\frac{(m-2)(m-1)}{2n}|-(n-m+2)\frac{(m-2)(m-1)}{2n}}\\
		&=\sum_{i=2}^{\ceil{\bar(l)}}{((m-i)-\frac{(m-2)(m-1)}{2n})} - \sum_{i=\floor{\bar(l)}}^{m-1}{((m-i)-\frac{(m-2)(m-1)}{2n})} \\ &-(n-m+2)\frac{(m-2)(m-1)}{2n}\\
	\end{align}
