...
\begin{proof}
	...
	Let $\min\lprof(x)(N)$ and $\max\lprof(x)(N)$ designate the minimal and maximal value of the loss vector of $x$.\commentBN{Why (N) ?}
	...
	Pick any $x \in \FBP$. 
	Pick any $i \in N$ such that $\lprofi[x] = \min\lprof(N)$ \commentBN{$\lprof(x)(N)$?} and define $\ibar$ as the other individual, thus with $\lprofibar[x] = \max\lprof(N) = \rho$. Define $z$ as the alternative such that $\lprofi[z] = \rho$.
	...
	Also, $\card{S_2} = m - 1 - \rho$. We obtain $m - 1 - \rho ≥ \rho - 1$, thus $\rho ≤ \frac{m}{2}$. And by definition of $\rho$, $\minineq[\FBP] ≤ \rho$. \commentBN{Why by definition of $\rho$?}
