...
We define $\Sigma^{C_{m,n}} \subseteq \Sigma$ as the set of spread measures that satisfy condition $C_{m,n}$ .
\commentRS{Discussion about the condition C that get stronger for larger m and n. Discuss the spread measures that exist in literature (this clarify why $\Sigma^{C_{m,n}}$ is never empty, $\sigma_{ad}$ is always there for $m,n \geq 3$).}
\subsubsection{Spread Measures in Literature}
...
\end{theorem}
\commentBN{For m=3 n=3 it doesn't work}
\begin{proof} 
...
\commentOC{Given a rule f, define property G as: if P has exactly one alternative that is never ranked last, f picks that alternative as a single winner. Theorem: Pick any m, n ≥ 3. There exists a profile P that has y as only alternative never ranked last, and under $Σ^C_{m, n}$, PCC requires that y be not a winner at P. Theorem 12 follows from this theorem because AP and FB satisfy G.}
