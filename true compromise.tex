...
\textit{Pareto-and-Veto rules (PV)} \citep{Laslier2020} distribute a veto power of $v_1$ and $v_2$ alternatives to voters 1 and 2, respectively, with $v_1+v_2=m-1$. So, every voter $i=1,2$ (simultaneously) vetoes his worst $v_i$ alternatives. The SCR picks all non-vetoed and Pareto optimal alternatives.
...
\begin{proof}
	...
	Therefore, $\sigma(\lambda_{P}(y)) < \sigma(\lambda_{P}(x))$. As $y$ is not Pareto-dominated, an SCR that uniquely picks $x$ at $P$ cannot be PCC. In a similar vein, at the profile $P'$ which is obtained by the inversion of the preferences of $i_1$ and $i_2$ at $P$, an SCR that is PCC cannot pick $x$ uniquely. \commentBN{I'm a bit confused by this phrasing, it seems that an SCR which picks $x$ and $y$ can be PCC. But $\sigma(y)$ is strictly less than $\sigma(x)$ so a PCC rule wouldn't pick $x$ at all.}
		\commentRS{As $\sigma(y)$ is strictly less than $\sigma(x)$, $x$ cannot be minimizing the loss vector. Thus, a SCR that uniquely picks $x$ cannot be PCC. Isn't this argument valid?}
	...
	For PV, let $v_1 ≥ v_2$ (thus $v_1 ≥ \frac{m-1}{2}$), and consider the profile $P$. Observe that the first voter vetoes at least $y$ and every $b_j$ ($1 ≤ j ≤ \beta$) while no voter vetoes $x$. As $x$ Pareto-dominates every $a_j$ ($1 ≤ j ≤ \alpha$), PV picks only $x$ at $P$. When $v_2 ≥ v_1$, a similar reasoning yields that PV picks only $x$ at $P'$. \commentBN{I don't get this, if we invert the veto power but we also invert the voters don't we reach the same situation? The voter whose top preference is $x$ is always the one having a greater veto power. What does this prove?}
	\commentRS{We invert the preferences of players and get another preference profile while we the veto powers are kept fixed. Does this answer your question? If this is not clear, let's clarify it.}
	
	Now let $m$ be odd.
	
	For VR, a reasoning similar to the one applied to PV yields $x$ as the unique choice at $P$: the last $\frac{m-1}{2}$ alternatives get vetoed and among the $\frac{m-1}{2}$ ones remaining, $x$ has the highest Borda score, at it is the only non-dominated alternative of that set, so it would be selected as the sole winner.
	\commentRS{When $m$ is odd, y and the b alternatives are vetoed by voter 1 and the a alternatives are vetoed by voter 2, thus $x$ is the only non-vetoed alternative. Am I right? If yes, this would simplfy the above argument, as well as the argument below for SL.}
	
	Finally, SL would also pick $x$, as it is chosen no matter which voter starts the veto phase. In fact if $i_1$ starts, she vetoes her worst $\frac{m-1}{2}$ alternatives, thus $y$ and every $b_j$ ($1 ≤ j ≤ \beta$), then $i_2$ chooses her best alternative out of the remaining ones which is $x$. If $i_2$ starts, she vetoes every $a_j$ ($1 ≤ j ≤ \alpha$), then $i_1$ chooses her best alternative which is $x$. 
\end{proof}
 
 \subsection{More on the two voters case}
 \begin{definition}
 	 A SCR $f$ satisfies the condition \textit{First Half} if it always picks one \commentBN{or more?} alternative which is ranked among the first half of both voters' preferences.
 \end{definition}
Please note that such an alternative always exists.

\begin{proposition} FB, PV (when the vetoes are equally distributed), SL and VR all satisfy \textit{First Half}.
\end{proposition}
\begin{proof}
	For FB, it is straightforward to see that, even if the two voters have opposite preferences, when descending level to reach unanimity we reach at most rank $\ceil{\frac{m}{2}}$.
...
Thus a SCR f is first half and Pareto iff f is a sub correspondence of PV.
FB, SL and VR are first half and Pareto, thus being (proper) sub correspondences of PV.

\begin{proposition} Under $\Sigma^{D_m}$ a SCR $f$ which is PCC does not satisfy first half.
\end{proposition}
