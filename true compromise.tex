...
\textit{Pareto-and-Veto rules (PV)} \citep{Laslier2020} distribute a veto power of $v_1$ and $v_2$ alternatives to voters 1 and 2, respectively, with $v_1+v_2=m-1$. So, every voter $i=1,2$ (simultaneously) vetoes his worst $v_i$ alternatives. The \ac{SCR} picks all non-vetoed and Pareto optimal alternatives. \commentBN{Moved this to: }\commentOC{ to present together the two rules that are defined for all $m$, then the two rules that are defined for $m$ odd; and to match the order used in the proof. Unless we find it more suitable to follow the chronological order.}
...
\begin{proof}
    ...
    Consider the profile $P$ where voter $i_1$ has the preference order $x > a_1 > … > a_\alpha > y > b_1 > … > b_\beta$ and voter $i_2$ has the preference order $b_1 > … > b_\beta > y > x > a_1 > … > a_\alpha$. \commentBN{Don't we want to use $\succ$?}
	
	\commentBN{Changed from this: 
	Consider the following profile P:
		\begin{center}
			$
		...
		\end{center}
	because: }
	\commentOC{This display may be misleading, because this alignment of alternatives is only correct for $m$ even (that is, when $\alpha = \beta + 1$).}
