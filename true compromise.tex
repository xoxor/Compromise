...
They don't, indeed! This observation was the basis for an objection made by Jean Fran\c{c}ois Laslier to the nomenclature on compromising.\footnote{This happened at a CNRS workshop on compromising hosted by Istanbul Bilgi University at Buyukada, Istanbul on fall 2018.} Consider 100 voters who all, except one, prefer an alternative $x$ to an alternative $y$ and $y$ to another alternative $z$ while the remaining voter prefers $z$ to $y$ and $y$ to $x$. So, with a display that we use throughout the paper, we have the
preference profile
...
where, except fallback bargaining, every member of the $q-$approval fallback bargaining class, including the majortarian compromise, will ignore the single voter and will pick $x$ as the collective outcome. As a matter of fact, almost every interesting SCR will ignore this \oquot marginal minority\cquot and choose $x$ in this situtation. While this choice is defendable on the grounds of qualified majoritarianism, the presence of $y$ which receives unanimous support when each voter falls back one step from his ideal point, renders questionable whether $x$ can be qualified as a compromise. The question becomes even more acute for majoritarian SCRs, including the majoritarian compromise, where $x$ would
remain the collective choice even when the split, rather than being 99-1, is 51-49, hence ignoring a minority of 49 percent. It is important to observe
that all these SCRs impose to voters a willingness to compromise which does not mean that under the collective choice that will be made, all voters
will be effectively compromising. In other words, the term \oquot compromise \cquot in this literature refers to procedural or \textit{ex-ante} compromises which is different than outcome oriented or \textit{ex-post} compromises, a conceptual distinction that seems to be overlooked in the literature.

To define an ex-post compromise, we adopt to our framework, a concept of equal losses that is prevalent in the literature that considers the allocation of continuous utilities.\footnote{This is the equal-loss principle used for bargaining problems (\cite{Chun1988}, \cite{Chun1991}) as well as for bankruptcy problems (\cite{Herrero2001}).} 
We introduce two definitions for being a compromise which ensure a collective choice where all voters \oquot equally \cquot give up from their ideal point. One of them, called \textit{egalitarian compromise}, insists on equality at the expense of Pareto efficiency while the other, called \textit{Paretian compromise}, is constrained to pick among the Pareto efficient alternatives. The two concepts are logically incompatible. As a result, Pareto efficient SCRs cannot ensure egalitarian compromises. Furthermore, large classes of SCRs of the literature such as Condorcet extensions, scoring rules, $q-$approval fallback bargaining, all fail to be Paretian compromises.
...
\commentBN{Moreover, given a vector $l \in \alllosses$, $\sigma(l)=\sigma(l')$ for any reordering $l'$ of $l$.}
...
\commentOC{TODO complement this remark.} \commentBN{I'm not quite sure what complement means here.}
...
%We call $f^{w}$ a \emph{strict scoring rule} whenever $w$ is
...
sufficiently large. \commentOC{We should introduce this alternative meaning of scoring rule that we use from here onwards, that is, a rule defined on a variable number of voters. And I’d favor using another word.} \commentBN{I don't recall the outcome of this discussion, actually I don't recall this discussion at all.}
...
%Remzi proof: First, observe the existence of $n_{1}\geq 3$ satisfying $k\cdot n_{1}\geq m-1$. Next, note that $\frac{n-1}{n}>w_{m-k}$ can be ensured with a sufficiently large $n$ if and only if $w_{m-k}<1$. So pick some $n_{2}\geq 3$ with $\frac{n_{2}-1}{n_{2}}>w_{m-k}$ . Taking $n=\max \left\{ n_{1},\text{ }n_{2}\right\} $, through Proposition \ref{prop:whenPCCfails}%, we see that $f^{w}$ fails PCC.
\end{proof}
...
$\forall i\in \left\{ 1,...,\text{ }m-1\right\} .$ \commentOC{This is actually an equivalent statement. And I find its interest clearer when written in the second way. Thus, I’d remove the previous proposition and directly prove it using the phrasing involving AP. And write it conversely: if  $f^w$ is PCC for any $n ≥ 3$, then $f^w$ is AP. Let’s write it $f^w_n$, by the way.} \commentBN{Again, I don't have in my notes the decision on this matter.}
...
%Considering any $x \in f_n(P)$, let us show that $x \in \mu_{\bar{\sigma}}^*(P)$. Because $x \in \paretopt(P)$, suffices to show that $\forall y \in \paretopt(P)$, $\bar{\sigma}(\lambda^P(y)) ≥ \bar{\sigma}(\lambda^P(x))$.
%Pick any $y \in \paretopt(P) \setminus \set{x}$. Because $\rho_n = \min_i r_\prefi(x)$, $\exists i \suchthat r_\prefi(y) > \rho_n$ (otherwise $\rho_n$ would be smaller). And, because $y \in \paretopt(P)$, $\exists j \suchthat \rho_n > r_{\pref_j}(y)$. We conclude that $\bar{\sigma}(\lambda^P(y)) ≠ 0$, thus $\bar{\sigma}(\lambda^P(y))  ≥ \bar{\sigma}(\lambda^P(x))$.
\end{proof}
