...
vector. We write $\Sigma$ for the set of spread measures $\sigma$ that satisfy for every $l\in\alllosses$, $\sigma(l)=0\iff l_{i}=l_{j}$ $\forall i,j\in N$. Thus, the spread of $l$ gets its lowest value $0$ in case of perfect equality and only in this case. \commentBN{Moreover, given a vector $l \in \alllosses$, $\sigma(l)=\sigma(l')$ for any reordering $l'$ of $l$.}
...
\commentOC{What do you think of dropping $\musigma$ and $\musigma$?}
...
\label{sec:BKn3}
Given any $r\in \intvl{1, m}$, \commentOC{Isn’t $r$ confusing here?} we write $n_{r}(x,P)=\#\{i\in
