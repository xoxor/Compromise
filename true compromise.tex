...
former case, $\mustar[\bar{\sigma }]=PO(P)$, and $\mustar[\bar{\sigma }]\cap $ $f^w(P)=\varnothing $ cannot hold, as the antiplurality rule, although not Paretian, never picks only non-Pareto optimal alternatives. So we consider the case $\bar{\sigma } (\lambda _{P}(x))=0$ $\forall x\in \mustar[\bar{\sigma }]$.
...
\commentRS{New proposal:}

\begin{theorem}
	Let $m\geq 3.$ Take any score vector $w.$ The SCR $f_{n}^{w}$ satisfies PCC for all $n\geq 3$ if and only if $w$ is the antiplurality score vector.
\end{theorem}

\begin{proof}
	Take any $m\geq 3$ and any score vector $w.$To show the "if" part, let $w$	be the antiplurality score vector and take any $n\geq 3.$CONTINUE\ WITH THE PROOF IN PROPOSITION 6. 
	
	To see the "only if" part, suppose $w$ is not the antiplurality score 	vector. So $w_{m-1}<1$. Observe the existence of two natural numbers $n_{1}$, $n_{2}\geq 3$ with $n_{1}\geq m-1$ and $\frac{n_{2}-1}{n_{2}}>w_{m-1}$.
	Let $n^{\ast }=\max \left\{ n_{1,}n_{2}\right\} $ and take any $n\geq	n^{\ast }.$Let $A=\left\{ a_{1}, \dots, a_{m}\right\} $. Take some $P\in
	L(A)^{N}$ with
	...
	\left\{ a_{m}\right\} $. Moreover, $a_{m}\in PO(P)$. Thus, $\mustar=\left\{ a_{m}\right\} $ $\forall \sigma \in \Sigma $. On the
	other hand, $s^{w}(a_{1}; P)=n-1$, $s^{w}(a_{m}; P)=n\cdot w_{m-1}$ and
	as $\frac{n-1}{n}>w_{m-k}$, we have $s^{w}(a_{1}; P)>s^{w}(a_{m};$ $P)$,
	establishing $a_{m}\notin f^{w}(P)$, thus $f^{w}(P)\cap \mustar=\varnothing $ $\forall \sigma \in \Sigma $.
\end{proof}

Remark that antiplurality fails PC.
...
\in \Sigma $ defined for each $(l_{1}, \dots ,l_{n})$ $\in \intvl{0,m-1}^N$ as $\bar{\sigma }(l_{1},...,l_{n})=1$ if 
$l_{i}\neq l_{j}$ for some $i,j\in N$ and $\bar{\sigma }(l_{1},\dots,l_{n})=0$ otherwise. 

Suppose, for a contradiction, $f_{n}$ fails PC. So,
there exist some $P\in $ $L(A)^{N}$ for which $f_{n}(P) \setminus \mustar[\bar{\sigma }]\neq \emptyset$ . Pick some $x\in
f_{n}(P) \setminus \mustar[\bar{\sigma }]$. As $x\in f_{n}(P)$, 
...
Given the choice of $\bar{\sigma}$, we have  $\bar{\sigma}(\lambda_{P}(y))=0$ and $\bar{\sigma}(\lambda_{P}(x))=1$. Thus, $r_{\succ _{i}}(y)=r_{\succ _{j}}(y)$ $\forall i,j\in N$. As $x\in f_{n}(P)$,
this contradicts $y\in PO(P)$.

\commentOC{Is it obvious that $x \in f_n(P)$ contradicts $y$ being same-ranked and PO?}
\commentBN{If such a $x$ exists, then the profile would be something like:
	\begin{center}
	$
	\begin{array}{cccccccccc}
	\mathbf{i_1} \quad &x&a_1&\dots&&a_k&y&b_1&\dots&b_k\\
	\mathbf{i_2} \quad &b_1&\dots&b_{k-1}&x&b_k&y&a_1&\dots&a_k\\
	\end{array}
	$
\end{center}
and $y$ is Pareto dominated by $x$.
}
