...
\begin{proposition} \label{prop:disjoint}
	\commentOC{We should determine for which values of $n$ and $m$ there exists $P$ with disjoint such sets.} \commentBN{}
...
\begin{proposition} \label{prop:negation}
	\commentOC{We should determine for which values of $n$ and $m$ this is true.} \commentBN{}
...
\begin{proposition}
	\commentOC{I think that we do not need to state the proposition that $ECC \not ⇒ PCC$, as it is a direct consequence of our other propositions: take a $f$ EC, it is also ECC, and it is not PCC.} \commentBN{I agree, but I also find nice to have a clear statement. Maybe we can write this as a corollary of proposition \ref{prop:negation}?}
...
\begin{proposition} \label{prop:POECC}
	$\nexists$ a Pareto Optimal SCR $f$ that is $ECC$. \commentBN{Maybe we can also write this as a corollary.}
\end{proposition}
...
\begin{proposition}
	Anti-plurality is not ECC.
\end{proposition}
\begin{proof}
	Banally consider the example in the proof of Proposition \ref{prop:disjoint}, where the most equal alternative $c$ is ranked last by all voters. This alternative is the only element of the set $\musigma$ but it is not selected by anti-plurality. 
\end{proof}

Note that, although we stated in Proposition \ref{prop:POECC} that no PO SCR is ECC, anti-plurality is not Pareto Optimal so it is worth to express this result.

\begin{proposition}
	Anti-plurality is not PC.
\end{proposition}
\begin{proof}
	...
	\begin{center}
		...
		$
	\end{center}
	The only element of the set $\musigma(P)$ is $a$, but anti-plurality selects $a$ and $b$.
\end{proof}

\begin{proposition}
	Anti-plurality is PCC.
\end{proposition}
\begin{proof}
	Take any profile $P$, then either
	\begin{itemize}
