...
\end{proposition}
	\begin{proof}
		In order to prove the proposition we need to show that $\exists$ a profile $P$ such that $\musigma(P) \cap \paretopt(P) = \emptyset$, $\forall \sigma \in \Sigma$.
...
It follows from the definitions that $\forall n, m ≥ 1, \quad \text{EC} \implies \text{ECC} \text{ and } \text{PC} \implies \text{PCC} $. Furthermore, from Proposition \ref{prop:nonParetian} we can deduce that, for $n\geq 2, \ m\geq3$, $\exists P$ such that $\musigma(P)$ and $\mustar(P)$ are disjoint  $\forall \sigma \in \Sigma$. 
...
\end{proposition}
\begin{proof}
%	We already showed, in the proof of Proposition \ref{prop:nonParetian}, that for $m=2$ and $\forall \sigma \in \Sigma$ and $\forall P \in \linors^N$ $\musigma(P) \cap \mustar(P) \neq 0$. So this proposition does not hold for $m<3$.
	Consider the profile $P$ described in the proof of Proposition \ref{prop:nonParetian}. \commentBN{Consider the new profile. And remzi's proof.}
	\begin{center}
		$
		\begin{array}{ccccc}
...
%	\commentBN{
%		\\ consider P the profile of 3.1, corollary 3.1 applies at that P : $\musigma \cap \mustar = 0 $ therefore $f(P)\cap\mustar(P)=0$
%		\\EC -> not PCC :
%		\\1. at that profile $\forall \sigma : f(P) \subseteq \musigma(P)$
%		\\2. }
\end{proof}
