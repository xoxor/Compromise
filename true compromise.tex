...
The condition is more convincing for bigger values of $m$ and $n$. In fact, asking for $\sigma(0,2,1)$ to be smaller than $\sigma(1,0,0)$ is very demanding while asking for $\sigma(3,5,4,4,4,4,4)$ to be smaller than $\sigma(6,5,4,3,2,1,0)$ reflects a mild assumption. In any case, as we state below, several well-known spread measures of the literature (see xxx for a comprehensive account) such as the  

\begin{itemize}
	\item the Gini coefficient, \[\sigma_{G}(r)= \frac{\sum_{i=1}^{n}\sum_{j=1}^{n}|r_i-r_j|}{2 \cdot n \cdot \sum_{i=1}^{n} r_i}.\]
...
\end{itemize} 
    all satisfy \cref{def:conditionC} for reasonably small values of m and n. 
    
    xxx
    The proofs for the following theorems can be found in \cref{apdx:proofSM}.
