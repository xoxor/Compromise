...
\citet{Mariotti1998} and \citet{Nagahisa2002} illustrate, the two worlds can be interconnected, as we do for the equal-loss principle of \citet{Chun1988} and \citet{Chun1991}. The arbitration environment presents an instance where envy-freeness matters and it is rather surprising to discover that several interesting \acp{SCR} used as arbitration solutions also fail to be Paretian compromises.
...
We analysed this remark experimentally. For each pair of $m \in \intvl{4,1000}$ and $n \in \intvl{l,1000}$, where $l=\max\{4,m-1\}$, we generated two vectors $v_1$ and $v_2$, of $n$ elements, of the form respectively $(m-3, m-1, m-2, \dots, m-2)$ and $(m-2, m-3, \dots, 1, 0, \dots, 0)$. We then computed their spread with the measures described above and made sure that $\sigma(v_1)<\sigma(v_2)$ for all $\sigma_{mad},\sigma_{ad},\sigma_{sd},\sigma_{G}$.
...
\begin{proof}
    ...
    For $m$ odd, note that $\alpha + \beta + 2 = m$ and consider the profile $P$ where voter $i_1$ has the preference order $x \succ a_1 \succ … \succ a_\alpha \succ y \succ b_1 \succ … \succ b_\beta$ and voter $i_2$ has the preference order $b_1 \succ … \succ b_\beta \succ y \succ x \succ a_1 \succ … \succ a_\alpha$. For $m$ even define the profile $P$ in the same way, except that a supplementary alternative $z$ is added at the bottom of both rankings; note that $\alpha + \beta + 2 + 1 = m$.
...
The exclusion of the equal-loss principle by almost all \acp{SCR} of the literature leads to ask whether the principle is uninteresting in a discrete social choice context. This seems to be the case for voting contexts where the number of voters exceeds the number of candidates and usually every candidate is ranked last by at least one voter. In such situations, picking an alternative whose "average rank" is highest is more of a concern than every voter giving up "equally". On the other hand, two-person collective choice problems are typically interpreted as arbitration or bargaining situations where mutual consent is a critical element in reaching a solution. As the vast literature on the ultimatum game \citep{Werner2014} suggests, mutual consent is hard to obtain when one individual sees injustice at the levels of mutual losses. Thus, the equal-loss principle appears to be valid for two-person collective choice problems and our analysis raises the question of designing new discrete arbitration rules compatible with the equal-loss principle. 
