...
\begin{definition}
    Given any $m \geq 4$ and $n=2$, we say that a spread measure $\sigma$ satisfies condition $D_m$ iff we have $\sigma(k+1,k-1)<\sigma(0,k)$ for some $k < m$.
\end{definition}
...
\begin{theorem}
	Let $m \geq 5$ and $\sigma \in \Sigma^{D_{m}}$. FB and PV fail PCC. Furthermore, if $m$ is odd, VR and SL also fail PCC. 
\end{theorem}
...
\begin{proof}
	Without loss of generality let us consider $m=2k+1$ an odd number of alternatives. Consider the following profile P:
		\begin{center}
			...
			$
		\end{center}
	We can note that $\sigma(\lambda_{P}(y)) = \sigma(k+1,k-1)$ and that $\sigma(\lambda_{P}(x))=\sigma(0, k)$. Therefore, $\sigma(\lambda_{P}(y)) < \sigma(\lambda_{P}(x))$ $\forall \sigma \in \Sigma^{D_{m}}$ and $\forall m\geq4$. In fact, since $m$ is odd, when we increment it to the next odd value, $k$ gets incremented by one, and an alternative $a_{k+1}$ and $b_{k+1}$ are added to the profile. This does not change the spread of the loss vector of the alternative $y$, while it increases the one of the loss vector of $x$. Any rule satisfying PCC would select $y$ since $\sigma(k+1,k-1)<\sigma(0,k)$ is true for some k.
	With the VR rule, the last $k$ alternatives get vetoed and, among the $k+1$ remaining, $x$ has the highest Borda score, so it would be selected as the sole winner. 
	The SL rule would also pick $x$ no matter which voter starts the veto phase.
	With PV, for any choice of $v_1$ and $v_2$ there exist a profile such that $y$ is vetoed by one of the two voters. \commentBN{Shouldn't we say instead that there is a choice of $v_1$ and $v_2$ such that in this specific profile $y$ is always vetoed?}
	FB would always pick $x$ since it is the first alternative which reaches the unanimous consent. 
	If $m$ is even we can consider to have an additional alternative $z$ ranked last by both voters. The same arguments for FB and PV hold.
\end{proof}
