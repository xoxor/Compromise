...
\commentOC{Please insert subsections for easier browsing.}
\begin{proof} for $\sigma_{ad}$. \\
	Recall that 
	\[\sigma_{ad}(l)= \frac{\sum_{i=1}^{n}|l_i-\bar{l}|}{n},\] 
	where $\bar{l}=\frac{\sum_{i=1}^{n}l_i}{n}$.
	Let us define $s_l= \sum_{i}|l_i-\bar{l}|$, so that $s(l) = n \smad(l)$.
	\commentOC{ad}
	\commentOC{$s_l$ or $s(l)$?}
	...
	\commentOC{from $i=1$}
	Recall that 
	\commentOC{$i$, not $x$. I think only the middle one is needed, and I suggest to write it $(m + 1 - k)$ in the first $()$ and remove the negative sign, that seems much more understandable to me.}
	\begin{align}
	...
	\end{align}
	If $m$ is even then $\floor{\bar{l_2}}=\bar{l_2}$ and we can write $s(l_{2 ev})$
	\begin{align}
...
Recall that now our thesis is $s(l_1)<s(l_2)$, or in this case that
	...
	\end{align}
If $m=4$ then $n\geq 4$, so if we prove that $s(l_1)<s(l_2)$ for $n=4$ then it is also true for any $n>4$.
\commentOC{I don’t see why “$s(l_1)<s(l_2)$ for any $n>4$” follows from “$s(l_1)<s(l_2)$ for $n=4$”. In general (for any expression on the LHS and RHS), it is false, so the reasoning must involve the specific expressions we got for $s$. For example, one can say, looking at $4n^2 < n + 100$, that this is true for $n = 4$, but it does not follow that it is true for all $n > 4$ as it fails for example for $n = 10$. I suggest to try to put all the terms that depend on $n$ on one side and proceed similarly as in the previous proof.}
\begin{align}
	&8(4)^2<(4-2)^2(4-1)^2 + 2(4)(4-2)(4-1)(2(4)-2(4)+3) \\
	&128<180.
\end{align}
 
If $m>4$ then $n\geq m-1$, so if we prove that $s(l_1)<s(l_2)$ for $n=m-1$ then it is also true for any $n>m-1$.
\begin{align}
	&8(m-1)^2<(m-2)^2(m-1)^2 + 2(m-2)(m-1)^2(2(m-1)-2m+3) \\
	&8(m-1)^2<(m-2)(m-1)^2 ( (m-2) + 2(2m-2-2m+3)) \\
	&8(m-1)^2<(m-2)(m-1)^2 ( m-2 + 2) \\
	&8(m-1)^2<m(m-2)(m-1)^2
\end{align}
Which is always true for $m>4$.
...
\begin{proof} for $\sigma_{sd}$. \\
	...
	where $\bar{l}=\frac{\sum_{i=1}^{n}l_i}{n}$.
	Let us define $s_l= \sum_{i}|l_i-\bar{l}|$, so that $s(l) = n \smad(l)^2$.
	Consider the two vectors $l_1=(m-3, m-1, m-2, \dots, m-2)$ and $l_2=(m-2, m-3, \dots, 1, 0, \dots, 0)$. The thesis is now that $s(l_1) < s(l_2)$.
