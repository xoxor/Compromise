...
where $k\in \left\{ 64,..., 99\right\}$ for the sake of the example. For any $q\in \left\{ \lfloor \frac{n}{2}\rfloor,..., k\right\}$ all the BK-compromises pick $b$ as winner. We can observe that, the outcome reflects a compromise among the first three blocks of voters that disagree on their top choices. The fourth block of $100-k$ voters, however, does not seem to be taken into account by any of the fallback bargaining rules. 
Let's consider $k=64$, the profile would then be the following: 
...
If we consider only the first level there are $26$ individuals voting for $a$, $25$ voting for $c$, $49$ voting for $d$, and none voting for $b$. No alternative reaches the quota, which is $q=64$, therefore we descend one level. Considering now the first two levels $a$ gets $62$ votes, $b$ gets $64$ votes, $c$ gets $25$ votes and $d$ gets $49$ votes. The social choice function would then select $b$, even though there is a group of $36$ voters for which the outcome is the least preferred alternative.
Let's consider the other extreme, $k=99$, the profile would then be the following: 
\begin{center}
	$
	\begin{array}{ccccc}
	\mathbf{26} \quad &a&b&c&d\\
	...
	$
\end{center}
The reasoning follows the same steps of the previous case, working now for any $q\in \left\{ 64,..., 99 \right\}$. 
...
\\\\
Let's define with $\Sigma_\text{threshold}$ the class of spread measures that consider the number of time the loss of an alternative is greater or equal than a certain threshold $k \in \R$:
\[\Sigma_\text{threshold}= \left\{ \sigma^k , k \in \R | \sigma^k (\lambda)= \# \{ u \in N, x \in A | \lambda_i(x) \geq k \} \right\}\]
\begin{proposition} 
\label{equivalence_SigmaThrs} 
Given any $\sigma^k \in \Sigma_\text{threshold} $ and any two UAs $v$ and $v^{\prime }$, there exists a $\sigma^{k^\prime}\in \Sigma_\text{threshold} $ such that $C^{\sigma }\circ v=C^{\sigma ^{k^\prime}}\circ v^{\prime }$. 
\end{proposition}
\begin{proof}
Define $\sigma^k$ by choosing a $k \in R$ which is greater than the $i_{th}$ biggest possible loss considering $v$; and a $\sigma^{k^\prime}$ such that $k^\prime$ is greater than the $i_{th}$ biggest possible loss considering now $v^\prime$. 
Let's take the following profile $P$ as an example:
	\begin{center}
		$
		\begin{array}{cccc}
		\mathbf{u_1} \quad &a&b&c\\
		\mathbf{u_2} \quad &b&c&a\\
		\end{array}
		$
	\end{center}
and consider the utility assignments $v$ such that $v_{P_1}=(3,1,0) , v_{P_2}=(0,3,1)$ and $v^\prime$ such that $v^\prime_{P_1}=(10,1,0) , v^\prime_{P_2}=(0,10,1)$. Define by $\lambda^P(x)$ the loss of utility of the alternative $x$ in the profile P under the utility assignment $v$, and $\lambda^{\prime P}(x)$ the one under the UA $v^\prime$. The possible values of $\lambda^P(x)$ are $(0,1,3)$ and the ones of $\lambda^{\prime P}(x)$ are $(0,9,10)$. Let's consider $\sigma^2$; $k=2$ is greater than the $2_nd$ biggest lost. We can construct $\sigma^{k^\prime}$ by choosing a $k^\prime$ greater than the $2_nd$ biggest lost considering $v^\prime$ and smaller or equal than the $3_rd$ one.
...
Note that by Proposition \ref{equivalence_SigmaThrs}, Definition \ref{D1} is equivalent to the apparently following stronger version:
