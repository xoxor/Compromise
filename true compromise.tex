...
In \cref{sec:more2voters} we focused on the analysis of voting rules when the number of voters involved into the decision process is greater than two. Keeping the notation introduced in \cref{sec:notation}, we consider here the case $n=2$. Two individuals express their preference over a set of alternatives $A$, and the goal is to find a common agreement on the alternative to select. This class of problems is often referred to as bargaining problems. We consider four prominent solutions of the literature:

We recall from \cref{sec:introduction} the definition of \textit{fallback bargaining (FB)} \citep{Brams2001}, a procedure that, starting from the top choices of each voter, it descends ranks until a unanimity consensus is found. Such alternative is selected as winner.

\textit{Veto Ranking (VR)} is commonly used in the selection of arbitrators (see \citep{Clippel2014}). Given a list of $m$ (odd) alternatives (that are candidates to be arbitrators), each of the two voters (that are the two parties that must agree on an arbitrator) simultaneously vetoes his worst $\frac{m-1}{2}$ alternatives. The selected alternatives are the ones with the highest Borda score among the non-vetoed alternatives, according to the voters' true preferences.

Again within the context of selecting arbitrators,\citet{Clippel2014} propose and analyze \textit{Shortlisting (SL)} where one of the two parties starts by selecting $\frac{m+1}{2}$ alternatives (thus vetoing the remaining ones), and then the second party chooses an alternative out of this subset. As the outcome of the procedure depends on the party that starts, symmetry among players is ensured by defining the solution as the union of the two outcomes where one and the other party starts.
...
\begin{proof}
	Without loss of generality let us consider $m=2k+1$ an odd number of alternatives. Consider the following profile P:
		\begin{center}
			...
			$
		\end{center}
	We can note that $\sigma(\lambda_{P}(y)) < \sigma(\lambda_{P}(x))$ $\forall \sigma \in \Sigma^{C_{m,2}}$ for some $k \in \N$. In fact, when $k$ gets incremented by one, an $a_{k+1}$ and a $b_{k+1}$ are added to the profile, thus not changing the spread of $y$. Therefore any PC $\sigma(k+1,k-1)<\sigma(0,k)$ which is true for some k.
