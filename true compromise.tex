...
Considering a loss vector, we are interested in measuring its "spread". We define a spread measure as a function $\sigma: \R_{+}^{N}\longrightarrow \R_{+}$ that associates a spread value to each possible loss vector.
We define $\Sigma$ as a set of spread measures such that for each $\sigma \in \Sigma$ and for each vector $l \in R_{+}^{N}$,  $\sigma(l_1, \dots, l_n)=0 \iff l_i=l_j$ $\forall i,j \in \{1, \dots, n\}$ . When all the values are the same, a coefficient of zero expresses perfect equality.
...
%	We already showed, in the proof of Proposition \ref{prop:disjoint}, that for $m=2$ and $\forall \sigma \in \Sigma$ and $\forall P \in L(A)^N$ $\musigma(P) \cap \mustar(P) \neq 0$. So this proposition does not hold for $m<3$.
	...
	Since $f$ is an EC, $f(P)={c}$, but $c$ is Pareto dominated, thus $\mu_{\sigma}^*(P)$ does not contain ${c}, \forall \sigma \in \Sigma$. So $f(P) \cap \mustar(P) = \emptyset$ and the first implication is proved. Moreover, we can observe that $\musigma(P)={c}$ and $c \notin \mustar(P)$, not only for some particular spread measure, but for all $\sigma \in \Sigma$. This proves the second implication.
\end{proof}
\begin{corollary} 
...
\commentBN{Do we have to define these rules first? Yes}
\begin{proposition}
...
\begin{proof}
	...
	In order to prove it for Anti-plurality, consider the profile described in the proof of Proposition \ref{prop:disjoint}, where the most equal alternative $c$ is ranked last by all voters. This alternative is the only element of the set $\musigma$ but it is not selected by anti-plurality. \commentBN{check after changing the example}
\end{proof}

\begin{proposition} For n>2, m=3 \commentBN{check for n=2 and m>3}
	\vspace{-4ex} 
	\begin{item}
...
\commentBN{Prop: Some scoring rules are  $PCC$}
