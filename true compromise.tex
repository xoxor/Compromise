...
Considering a loss vector, we are interested in measuring its \oquot spread\cquot. We define a spread measure as a function $\sigma: (0,\dots,m-1)^{N}\longrightarrow \R_{+}$ that associates a spread value to each possible loss vector.
We call $\Sigma$ the set of spread measures $\sigma$ such that for each vector $l \in (0,\dots,m-1)^{N}$,  $\sigma(l_1, \dots, l_n)=0 \iff l_i=l_j$ $\forall i,j \in \{1, \dots, n\}$ . Thus, the spread of $l$ gets its lowest value $0$ in case of perfect equality and only in such case.
...
\begin{proposition} \label{prop:disjoint}
	For $n\geq 2, \ m\geq3,$ there $\exists P$ such that $\musigma(P)$ and $PO(P)$ are disjoint.
\end{proposition}
	\begin{proof} 
		...
		\begin{center}
			$
			\begin{array}{cccccc}
			\mathbf{1} \quad &a_1&a_2&a_3&\dots&a_m\\
			\mathbf{n-1} \quad &a_2&a_3&\dots&a_1&a_m\\
			\end{array}
			$
		\end{center}
		where we have $m$ alternatives disposed such that only one alternative $a_m$ is ranked at the same position by every voter, and it is ranked last.
		For each $\sigma \in \Sigma$ the spread associated to every alternative except $a_m$ is greater than zero $\sigma(\lambda^P(a))>0$, $\forall a \in A \setminus \{a_m\}$; whilst $\sigma(\lambda^P(a_m))=0$. Thus, the set $\musigma(P)$ of the \oquot most equal\cquot \ alternatives consists of the only element $a_m$. On the other hand, the set $\mustar(P)$ will never contain $a_m$ whatever is the choice of $\sigma$ since it is a Pareto dominated alternative.
	\end{proof}
\begin{note}
...
%	We already showed, in the proof of Proposition \ref{prop:disjoint}, that for $m=2$ and $\forall \sigma \in \Sigma$ and $\forall P \in \linors^N$ $\musigma(P) \cap \mustar(P) \neq 0$. So this proposition does not hold for $m<3$.
	Consider the profile $P$ described in the proof of Proposition \ref{prop:disjoint}. \commentBN{Consider the new profile.}
	\begin{center}
		$
		\begin{array}{ccccc}
...
\begin{corollary} \label{cor:optECC}
	\commentBN{m,n}$\nexists$ Pareto Optimal SCR $f$ that is $ECC$.
\end{corollary}
...
\begin{proof}
	...
	In order to prove it for Anti-plurality, consider the profile described in the proof of Proposition \ref{prop:disjoint}, where the most equal alternative $z$ is ranked last by all voters. This alternative is the only element of the set $\musigma$ but it is not selected by anti-plurality. \commentBN{Change according to the new profile.}%profile st z is the last and the other are shifted by 1}
\end{proof}
