...
From these definitions, it is plain that being an EC implies being ECC.

\begin{theorem} \label{prop:nonParetian}
	For $n\geq 2, \ m\geq3$, no Paretian SCR is $ECC$.
\end{theorem}
\begin{proof}
	In order to prove the proposition we need to show that $\forall \sigma \in \Sigma$, for some profile $P$, $\paretopt(P) \cap \musigma(P) = \emptyset$.
...
\commentRS{For Proposition 3.2, the proof of PC implies not ECC is straightforward: if f is PC, then f is Paretian and by Proposition 3.1, f is not ECC. }
