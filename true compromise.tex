...
\end{proposition}
\commentBN{We checked experimentally and analytically.}
...
\commentRS{I have a trivial answer: Our condition in Definition 5 is not defined for $m=3$. So for $m=3$, the set of spread measures is unrestricted. Thus, Theorems 5 and 7 apply. Does this make sense?}

\section{Two voters case}
\commentBN{This is often referred to as bargaining or arbitration problem. In some of them is precisely defined a disagreement point ["Bargaining over a finite set of alternatives."], some do not and we consider this.}
...
\end{definition}
For $m=7$ the condition requires $\sigma(4,2) < \sigma(0,3)$ and $\sigma(2,4) < \sigma(3,0)$ which is reasonable in our context. \commentRS{I made a correction in the preceding sentence, please check} When the value of $m$ is larger, the condition appears even more convincing. As $m$ grows, the distance between $0$ and $\floor{\frac{m}{2}}$ grows, while the distance between $\ceil{\frac{m}{2}}$ and $\floor{\frac{m}{2}} - 1$ remains constant ($1$ if $m$ is even, $2$ otherwise). Requiring, for example, the spread of $(15,14)$ to be smaller than the spread of $(0,15)$ is very reasonable.
...
\begin{proof}
	...
	\commentBN{Add that when m is even the proof works by adding an alternative c at the end.}
\end{proof}
