...
\commentBN{Discussion 23/09/2020}
\begin{example} $2k+2$ alternatives
	\begin{center}
		$
		\begin{array}{cccccccccc}
		\mathbf{i_1} \quad &x&a_1&\dots&&a_k&y&b_1&\dots&b_k\\
	...
	\end{center}
\end{example}
FB picks x, what would our approach pick? The response is not immediate because even if y seems the most equal the spread is not 0. lambda(x)=(0,k), lambda(y)=(k-1,k+1). There are some values of k such that lambda(y)<lambda(x). Except y everything grows with k. A Th. would be that FB is not compatible with the compromise idea. What Clippel (paper "On the selection of arbitrators") would pick? Whoever starts to veto $k+1$ alternative it would pick $x$ again. Pareto and veto rule: the veto power is distributed such that $v_1+v_2=m-1$. I have to distribute a veto power of $2k+1$ to the two voters, so the best I can do is to give $k$ and $k+1$. If $i_1$ vetoes $k+1$ is FB, if she vetoes $k$ both $x$ and $y$ are elected.
What happens if we have $2k+3$? then $x,y$ are both elected.
We could have a result such that when we have an even number of alternatives there is a distribution of alternatives st is PCC.
Clippel criticizes an approach and propose a new one, we take both, we take Pareto and veto and FB and we compare to our approach.

\commentBN{Discussion 01/10/2020}
\begin{theorem}
    Take m odd, none FB VR SL and PV don't satisfy PCC
\end{theorem}
\begin{example} $2k+1$ alternatives
	\begin{center}
		$
		\begin{array}{cccccccccc}
	...
	\end{center}
\end{example}
When $k$ grows (we add one more $a$ and one more $b$), the spread of $y$ doesn't change. 
$\sigma(k+1,k-1)<\sigma(0,k)$ which is true for some k.
With the VR rule, the last $k$ alternatives get vetoed and, among the $k+1$ remaining, $x$ has the highest Borda score. 
The SL rule picks $x$.
With PV, for each $v_1$ and $v_2$ there exist a profile such that $y$ is excluded. 

Justification for the rule: ultimatum game. People are willing to give up even more than our rule.
