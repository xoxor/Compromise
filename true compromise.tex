...
\commentOC{This should be $\min_{\sigma \circ \lambda_P} X = \set{x \in X \suchthat \forall y \in X: …}$, right?}
...
\begin{proof}
	...
	\commentOC{Do we mean that $\sigma(\lambda_P(a)) > 0$?}
\end{proof}
...
\end{theorem}
\begin{proof}
Take any score vector $w$. We say that $w$ is \emph{strict} iff $w_{i}>w_{i+1}$ $\forall i\in \intvl{1, m-1}$. If $w$ is strict, then $f^w$ is Paretian and \cref{th:nonParetian} implies that $f^w$ is not ECC. Now, let $w$ be non-strict. Consider the profile $P$ of \cref{prop:muSigmaLast}. Observe that $\musigma=\{a_m\} \ \forall \sigma \in \Sigma $. However, as $w_{1}>w_{m}$, we have $s^{w}(a_{1},P)>s^{w}(a_{m},P)$ which implies $a_{m}\notin f^{w}(P)$.
	\commentOC{I suggest to remove “We say that $w$ is \emph{strict} iff $w_{i}>w_{i+1}$ $\forall i\in \intvl{1, m-1}$. If $w$ is strict, then $f^w$ is Paretian and \cref{th:nonParetian} implies that $f^w$ is not ECC. Now, let $w$ be non-strict.”}
\end{proof}
...
\commentOC{I suggest this alternative wording, which seems more direct to me.}
\begin{proof}
...
%	We then have that $x \in \mustar[\bar\sigma]$ as by definition of $\bar\sigma$, $\bar\sigma(x) ≤ 1$.
	...
	If $p = 0$, take any $x \in \mustar[\bar\sigma]$. As $\bar\sigma (\lambda _{P}(x))=0$, we have, $\forall i, j \in N$: $\lambda_i^P(x) = \lambda_i^P(x)$, hence, $\forall i, j \in N$: $r_{\succ_i}(x) = r_{\succ_j}(x)$. 
...
In the years many measures of inequality have been proposed (\cite{Jackman1974}; \cite{Rubinson1977}; \cite{Martin1971}; \cite{Ray1973}) but perhaps the most commonly used is the Gini coefficient, G, which is defined as the ratio of the area between the Lorenz curve and the diagonal (line of equality), over the total area under the line of equality.
\commentOC{I’d use citep here.}
