...
\begin{definition}
	...
	\commentOC{Even though the condition is indeed well-defined for $m = 3$, and technically the theorem of interest can be proven for $m = 3$, I think we should start at $m = 4$. Because the condition is not reasonable to impose for $m = 3$. This then opens a question about the case $m = 3$.}
\end{definition}
...
\begin{definition}
    Given any $m \geq 6$, a spread measure $\sigma \in \Sigma$ satisfies condition $D_m$ iff 
    $\sigma(\ceil{\frac{m}{2}}, \floor{\frac{m}{2}} - 1) < \sigma(0, \floor{\frac{m}{2}})$,
    $\sigma(\floor{\frac{m}{2}} - 1, \ceil{\frac{m}{2}}) < \sigma(\floor{\frac{m}{2}}, 0)$,
    $\sigma(\floor{\frac{m}{2}} + 1, \floor{\frac{m}{2}} - 1) < \sigma(0, \floor{\frac{m}{2}})$ and
    $\sigma(\floor{\frac{m}{2}} - 1, \floor{\frac{m}{2}} + 1) < \sigma(\floor{\frac{m}{2}}, 0)$.
\end{definition}
For $m=6$ the condition requires $\sigma(3, 2) < \sigma(0, 3)$, $\sigma(2, 3) < \sigma(3, 0)$, $\sigma(4, 2) < \sigma(0, 3)$ and $\sigma(2, 4) < \sigma(3, 0)$, which is reasonable in our context. When the value of $m$ is larger, the condition appears even more convincing. As $m$ grows, the distance between $0$ and $\floor{\frac{m}{2}}$ grows, while the distance between $\ceil{\frac{m}{2}}$ and $\floor{\frac{m}{2}} - 1$, or between $\floor{\frac{m}{2}} + 1$ and $\floor{\frac{m}{2}} - 1$, remains constant. Requiring, for example, the spread of $(15, 14)$ to be smaller than the spread of $(0, 15)$ is very reasonable.
\commentOC{This is becoming absurd. I suggest to request that $\forall \abs{k - k'} ≤ 2: \sigma(k, k') < \sigma(0, \floor{\frac{m}{2}})$ and $\sigma(k, k') < \sigma(\floor{\frac{m}{2}}, 0)$.}

We write $\Sigma^{D_{m}} \subseteq \Sigma$ for the set of spread measures that satisfy condition $D_{m}$. 

\begin{figure}
	\caption{The profile $P^\text{FB}$ as used in the proof of \cref{th:2votPCC}, for $m = 6$.}
	\label{fig:Pfb6}
	$\begin{array}{llllll}
		x & a_1 & a_2 & b_1 & y & b_2\\
		b_1 & b_2 & y & x & a_1 & a_2\\
...
\end{figure}
\begin{figure}
	\caption{The profile $P^\text{FB}$ as used in the proof of \cref{th:2votPCC}, for $m = 7$.}
	\label{fig:Pfb7}
	$\begin{array}{lllllll}
...
\begin{theorem} 
	\label{th:2votPCC}
	Let $m \geq 6$. Under $\Sigma^{D_{m}}$, FB and PV fail PCC. Furthermore, when $m$ is odd, VR and SL also fail PCC.
\end{theorem}
\begin{proof}
	Take any $m \geq 6$ and any $\sigma \in \Sigma^{D_m}$. Define $\alpha = \ceil{\frac{m}{2}} - 1$ and $\beta = \floor{\frac{m}{2}} - 1$. Note that $\sigma(\alpha + 1, \beta) < \sigma(0, \beta + 1)$ and $\sigma(\beta, \alpha + 1) < \sigma(\beta + 1, 0)$, and $\alpha + \beta + 2 = m$.
	Define $\succ^m_1$ as the preference order $x \succ a_1 \succ … \succ a_\alpha \succ y \succ b_1 \succ … \succ b_\beta$, $\succ^{m\text{-rev}}_1$ as the preference order $x \succ a_1 \succ … \succ a_\alpha \succ b_1 \succ y \succ b_2 \succ … \succ b_\beta$ (thus with the order of $b_1$ and $y$ reverted compared to $\succ^m_1$), and $\succ^m_2$ as the preference order $b_1 \succ … \succ b_\beta \succ y \succ x \succ a_1 \succ … \succ a_\alpha$. 
	
	Consider the profile $P^\text{FB}$ where voter $1$ has the preference $\succ^m_1$ if $m$ is odd and the preference $\succ^{m\text{-rev}}_1$ if $m$ is even; and where voter $2$ has the preference $\succ^m_2$. \Cref{fig:Pfb6} displays $P^\text{FB}$ for $m = 6$ and \cref{fig:Pfb7} for $m = 7$.
	
	Note that $\lambda_{P^\text{FB}}(y) = (\floor{\frac{m}{2}} + 1, \beta)$, $\lambda_{P^\text{FB}}(x) = (0, \beta + 1)$ and, if $m$ is even, $\lambda_{P^\text{FB}}(b_1) = (\alpha + 1, 0)$. We see that FB picks $x$, together with $b_1$ when $m$ is even, as winners, whereas none of these minimize $\sigma \circ \lprof$ among the Pareto optimal alternatives, $y$ reaching a lower inequality value thanks to condition $D_m$. Thus, FB is not PCC. BUT $y$ is not PO!
...
\begin{proof} for $\sigma_{ad}$. \\
	...
	If $m$ is even then $\floor{\bar{l_2}}=\bar{l_2}$ 
	\commentOC{This had escaped me previously. I could have misunderstood something, but it seems false that “If $m$ is even then $\floor{\bar{l_2}}=\bar{l_2}$”. (And I don’t see why it would depend on the parity of $m$.) For example, with $m = 4$ and $n = 5$, $\bar{l_2} = 0.6 ≠ \floor{\bar{l_2}}$. One approach could be, rather, to define $\epsilon = \bar{l_2} - \floor{\bar{l_2}}$, thus $0 ≤ \epsilon < 1$, and replace $\floor{\bar{l_2}}$ by $\bar{l_2} - \epsilon$ in the expression below. I suppose that it will appear that epsilon is small enough to not matter.}
	and we can write $s(l_{2 ev})$
	\begin{align}
		&=\bltwo(2\bltwo-\bltwo-1)+2\bltwo(n-m+2)=\\
		&=\bltwo(\bltwo-1 +2(n-m+2))= \tag{2}\\
		&=\frac{(m-2)(m-1)}{2n}(\frac{(m-2)(m-1)}{2n} +2n-2m+3)
	\end{align}
Recall that now our thesis is $s(l_1)<s(l_2)$, or in this case that
	\begin{align}
		&2<\frac{(m-2)(m-1)}{2n}(\frac{(m-2)(m-1)}{2n} +2n-2m+3)\\
		&4n<(m-2)(m-1)(\frac{(m-2)(m-1)}{2n} +2n-2m+3) \\
		&4n<\frac{(m-2)^2(m-1)^2}{2n} + (m-2)(m-1)(2n-2m+3) \\
		&8n^2<(m-2)^2(m-1)^2 + 2n(m-2)(m-1)(2n-2m+3) \\
	\end{align}
If $m=4$ then we have:
	\begin{align}
		&8n^2<(4-2)^2(4-1)^2 + 2n(4-2)(4-1)(2n-2(4)+3) \\
		&8n^2<4 \cdot 9 + 2n \cdot 2 \cdot 3 (2n-8+3) \\
		&8n^2< 36 + 12n (2n-5) \\
		&8n^2< 36 + 24n^2 -60n \\
		&8n^2 - 24n^2 + 60n - 36 < 0   \\
		&- 16n^2 + 60n - 36 < 0   \\
		& 4n^2 - 15n +9 > 0   \\
	\end{align}
When $m=4$, $n\geq 4$ and this inequality holds.
If $m>4$ then $n\geq m+1$ \commentOC{$m - 1$}, let us consider $n=m+1$ first:
	\begin{align}
		&8(m+1)^2<(m-2)^2(m-1)^2 + 2(m+1)(m-2)(m-1)(2(m+1)-2m+3) \\
		&8(m+1)^2<(m-2)^2(m-1)^2 + 2(m+1)(m-2)(m-1)(2m+2-2m+3) \\
		&8(m+1)^2<(m-2)^2(m-1)^2 + 10(m+1)(m-2)(m-1) \\
		&0<\frac{(m-2)^2(m-1)^2}{8(m+1)^2} + \frac{10(m+1)(m-2)(m-1)}{8(m+1)^2} \\
		&0<\frac{(m-2)^2(m-1)^2}{8(m+1)^2} + \frac{10(m-2)(m-1)}{8(m+1)} \\
	\end{align}
Since all the terms in left-hand side are positive the inequality holds.
\commentBN{I don't get what can we say about $n>m+1$. Is this already enough?}
...
We still need to consider the case where $m$ is odd. In this case we can say that $\fltwo>\bltwo-1$, so we can continue from Equation $(1)$ 
\begin{align}
	&(1)>(\bltwo-1)(2\bltwo-(\bltwo-1)-1)+2\bltwo(n-m+2)\\
	&(1)>(\bltwo-1)(2\bltwo-\bltwo+1-1)+2\bltwo(n-m+2)\\
	&(1)>\bltwo(\bltwo-1)+2\bltwo(n-m+2)\\
\end{align}
The right side of this inequality is the same equation $(2)$ obtained for the case $m$ even. Thus, since we proved that $s(l_1)<s(l_{1 ev})$ we can conclude that $s(l_1)<s(l_{2 ev})<s(l_{2 odd})$.
\end{proof}
