...
\end{theorem}
\begin{proof}	
	Letting $\Ptop$ \commentOC{Why not simply $P$?} denote the profile of \cref{prop:muSigmaLast}, with $a_m$ the alternative mentioned there, and considering any EC $f$ and any $\sigma \in \Sigma$, suffices to prove that $f(\Ptop) \cap {\mustar[\sigma][\Ptop]} = \emptyset$.
...
\begin{proof}
	...
	\commentOC{Here (and in the next section) we use $\min$ in (what I believe to be) the most usual sense, whereas elsewhere we use $\min$ instead of $\argmin$. Should we change all these $\min$ to $\argmin$ (my favorite option)? Or should we say that we abuse notation?}
	...
	By definition of $\bar\sigma$, $\bar\sigma(x) ≤ 1$, hence, $x \in \mustar[\bar\sigma]$ \commentOC{Why “$\bar\sigma(x) ≤ 1$, hence”? I propose to write: “As $k = 1$, $x \in \mustar[\bar\sigma]$.”}.
...
\begin{proof}
	Take any $m\geq 3$ and any score vector $w$ that is not the antiplurality score vector. Therefore, $w_{m-1}<1$. Observe the existence of two natural numbers $n_{1}$, $n_{2}\geq 3$ with $n_{1}\geq m-1$ and $\frac{n_{2}-1}{n_{2}}>w_{m-1}$. \commentOC{Very minor, but I find the observation of the existence phrasing and the indirect definition of $n$ inelegant; I’d prefer: Pick any $n$ such that $n ≥ m - 1$ and $n > \frac{1}{1 - w_{m - 1}}$.}
	...
	as $\frac{n-1}{n}>w_{m-1}$ \commentOC{I’d write: as $n > \frac{1}{1 - w_{m - 1}}$ (and specifying, “equivalently, $n - 1 > n w_{m - 1}$” if you think it is appropriate).}, we have $s^{w}(a_{1}; P)>s^{w}(a_{m};$ $P)$,
...
\ac{SCR} $f_{q}$ defined for each $P\in \linors^N$ as $f_{q}(P)=\{x\in A \suchthat n_{\rho _{q}(P)}(x,P)\geq n_{\rho _{q}(P)}(y,P)$ $\forall y\in A\}.$ \commentOC{Shouldn’t we also say BK compromise in the introduction, then?}
