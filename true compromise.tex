...
We denote the minimal elements of $X\in2^{A}\setminus \{\emptyset\}$ according to $(\sigma\circ\lambda_{P})$ with $\min_{\sigma \circ \lambda_P} X = \set{x \in X \suchthat \forall y \in A: \sigma(\lambda_P(x)) ≤ \sigma(\lambda_P(y))}$. Thus, $\min_{\sigma\circ\lambda_{P}}(X)$ denotes the alternatives in X whose loss vectors are the most equally distributed according to the spread measure $\sigma$.
\commentOC{This should be $\min_{\sigma \circ \lambda_P} X = \set{x \in X \suchthat \forall y \in X: …}$, right?}
\commentRS{Yes, I corrected}
...
\begin{proof}
	...
	\commentOC{Do we mean that $\sigma(\lambda_P(a)) > 0$?} 	\commentRS{Yes, I corrected}
\end{proof}
