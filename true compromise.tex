...
We call $\Sigma$ the set of spread measures $\sigma$ such that for each vector $l \in R_{+}^{N}$,  $\sigma(l_1, \dots, l_n)=0 \iff l_i=l_j$ $\forall i,j \in \{1, \dots, n\}$ . Thus, the spread of $l$ gets its lowest value $0$ in case of perfect equality and only in such case.
...
\begin{proof}
	...
	In order to prove it for Anti-plurality, \commentBN{profile st z is the last and the other are shifted by 1} consider the profile described in the proof of Proposition \ref{prop:disjoint}, where the most equal alternative $z$ is ranked last by all voters. This alternative is the only element of the set $\musigma$ but it is not selected by anti-plurality.
\end{proof}
