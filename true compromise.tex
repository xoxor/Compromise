...
Consider a finite set $N$ of individuals with $\#N=n\geq 2$ and a finite set $A$ of alternatives with $\#A=m\geq 2$. We write $L(A)$ for the set of linear orders over $A$ and $P_{i}\in L(A)$ stands for the preference of $i\in N$. An ordinal social choice rule (SCR) is a mapping $f:L(A)^{N}\rightarrow 2^{A} \setminus \{\emptyset \}$.
...
We start by two examples, based on an observation made by Laslier at Buyukada, that will help us to define the problem and to give the motivation for our approach.
\begin{example}
	\label{ex:ex1}
	Consider the following preference profile $P\in L(A)^{N}$ with $|N|=n$ voters:
	\begin{center}
		$
		\begin{array}{cccc}
...
\section{Compromises}
Consider a set of alternative $A$, a set of voters $N$, a profile $P$ and a SCR $f$ as defined above. For each $x \in A$ and $i \in N$ let $r^{P_i}(x)$ be the rank of the alternative $x$ in the preference order of $i$; we write with $\lambda^{P_i}(x)=r^{P_i}(x)-1$ the loss of ranks for $i$ when electing $x$ instead of her favorite alternative. This value can also be seen as the number of pairwise flips we need to perform in order to place $x$ at top position for $i$. Thus, $\lambda ^{P}(x)=(\lambda _{i}^{P}(x))_{i\in N}\in \R_+^N$ represents the vector of losses when electing $x$. 
We define a spread measure $\sigma \in \SAll$ as a function $\sigma : \R_{+}^{N}\longrightarrow \R_{+}$ such that $\sigma(r_i, \dots, r_n)=0 \iff r_i=r_j \forall i,j$. When all the values are the same, a coefficient of zero expresses perfect equality.
...
\begin{remark}
	Let $\paretopt(P)$ be the set of alternatives which are Pareto optimal at $P$. $\paretopt(P)$ and the set $f(P)$ of the alternatives elected by a compromise SCR $f$ might be disjoint.
...
Consider again the profile $P$ of the Example \ref{ex:ex1}. 
	\begin{center}
		$
		\begin{array}{cccc}
		\mathbf{n-1} \quad &a&b&c\\
		...
		$
	\end{center}
	At $P$, $b$ is the only efficient compromise, and we can say that 
\begin{itemize}
	\item Condorcet is not an efficient compromise since it picks $a$ as the winner.
	\item No scoring rule except anti-plurality is an efficient compromise
	\begin{proof} $ $ \\
		Without loss of generality we can consider the score function $s:\{1,\dots,m\}\rightarrow [0,1]$ a function assigning $1$ to the alternative ranked at the first position, and $0$ to the one ranked at the last.
		\begin{center}
			$
			\begin{array}{ccc}
			\quad & \mathbf{k} &\mathbf{1}\\
			1 \quad &a&c\\
			\lambda \quad &b&b\\
			0 \quad &c&a\\
			\end{array}
			$
		\end{center}
		We have that $s(a)=k$, $s(b)=(k+1)\cdot \lambda$, $s(c)=1$. For $a$ to be selected as winner it must be that $k>(k+1)\cdot\lambda$ so that $k>\frac{\lambda}{(1-\lambda)}$, and that is always the case unless $\lambda=1$ which is anti-plurality.
	\end{proof}
	\item No BK-compromise is an efficient compromise, unless $q=n=k+1$ which is fallback bargaining.
	\item No preferential runoff method is an efficient compromise.
\end{itemize}	
...
\end{definition}
We write $\SPPd \subseteq \SAll$ for the class of spread measures that satisfy also PPd. Any $\sigma \in \SPPd$ reaches a minimum value for an input whose components are all equal.
...
\begin{proof} 
	...
	The remaining term $\frac{1}{n^2}$ is constant and same for both, so $\sigma_{mad}(r) \leq \sigma_{mad}(s)$ and PPd is satisfied.
\end{proof}
\begin{example}
