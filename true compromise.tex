...
Let $U(A) \subseteq \R^A$ be the set of real-valued “utility” functions defined over $A$ with $u_{i}(x)\neq u_{i}(y)$ $\forall x,y\in A$, $\forall u_{i}\in U$. \commentOC{Is $U(A)$ meant here? Otherwise, I do not understand what $U$ is.} Note that each $u_{i}\in U$ induces a unique $P_{i}(u_{i})\in L(A)$ \commentOC{I suggest writing $P(u_i)$.}. For each $k\in \left\{ 1, ..., m\right\} $, we write $r_{k}(u_{i})$ for $r_{k}(P_{i}(u_{i}))$.
...
However, from now on, we will ensure the Pareto optimality of cardinal compromises by defining them as $C^{\sigma }(u)=\left\{ x\in PO(u):\sigma (\lambda ^{u}(x))\leq \sigma (\lambda ^{u}(y)), \forall y\in A\right\}$. \commentOC{Damned, my communist propaganda failed again.}
