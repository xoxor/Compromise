...
\]
\commentOC{I suggest the following notation instead. Given $X \subseteq A$, define $\min_{\sigma \circ \lambda^P} X = \set{x \in X \suchthat \forall y \in A: \sigma(\lambda^P(x)) ≤ \sigma(\lambda^P(y))}$ as the minimal elements of $X$, as measured by $(\sigma \circ \lambda^P)$. Then, $\musigma(P) = \min_{\sigma \circ \lambda^P} (A)$ and $\mustar(P) = \min_{\sigma \circ \lambda^P} (\paretopt(P))$.}
...
We are interested in this section in analyzing the following classes of social choice rules. \emph{All} Egalitarian Compromises winners are among the alternatives with most equally distributed losses; while \emph{some} Egalitarian Compromise Compatible winners are among the alternatives with most equally distributed losses.
...
\commentOC{TODO complement this remark.}
\begin{remark}
	\Cref{prop:nonParetian} does not hold for $m=2$: in that case, the largest Paretian SCR, $f(P) = \paretopt(P)$, is ECC. In fact, $\forall \sigma, P: \paretopt(P) \cap \musigma(P) ≠ \emptyset$. This is because all profiles with two alternatives have this form, with $k < \frac{n}{2}$:
	\begin{center}
		$
		\begin{array}{ccc}
		\mathbf{k} \quad &a&b\\
...
\end{proposition}
\commentOC{Didn’t we have some similar result whose proof was easier? Let’s try to simplify it, under condition that the other results still follow.}
\begin{proof}
...
we have scoring vectors with $w_{2}<1$ which also includes the Borda rule. \commentOC{Improve wording and give a reference.}
...
\end{proposition}
\begin{proof}
\Cref{whenPCCfails} can be applied by taking $k=m-2$ and observing that $n ≥ 3 ≥ \frac{m-1}{m-2}$.
\end{proof}
