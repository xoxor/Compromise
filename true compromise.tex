...
\end{theorem}
\begin{proof}	
	Letting $\Ptop$ \commentOC{Why not simply $P$?} \commentRS{I agree} denote the profile of \cref{prop:muSigmaLast}, with $a_m$ the alternative mentioned there, and considering any EC $f$ and any $\sigma \in \Sigma$, suffices to prove that $f(\Ptop) \cap {\mustar[\sigma][\Ptop]} = \emptyset$.
...
\begin{proof}
	...
	\commentOC{Here (and in the next section) we use $\min$ in (what I believe to be) the most usual sense, whereas elsewhere we use $\min$ instead of $\argmin$. Should we change all these $\min$ to $\argmin$ (my favorite option)? Or should we say that we abuse notation?}\commentRS{Let's go for your favorite option}
	...
	By definition of $\bar\sigma$, $\bar\sigma(x) ≤ 1$, hence, $x \in \mustar[\bar\sigma]$ \commentOC{Why “$\bar\sigma(x) ≤ 1$, hence”? I propose to write: “As $k = 1$, $x \in \mustar[\bar\sigma]$.”}.\commentRS{I agree}
...
\begin{proof}
	Take any $m\geq 3$ and any score vector $w$ that is not the antiplurality score vector. Therefore, $w_{m-1}<1$. Observe the existence of two natural numbers $n_{1}$, $n_{2}\geq 3$ with $n_{1}\geq m-1$ and $\frac{n_{2}-1}{n_{2}}>w_{m-1}$. \commentOC{Very minor, but I find the observation of the existence phrasing and the indirect definition of $n$ inelegant; I’d prefer: Pick any $n$ such that $n ≥ m - 1$ and $n > \frac{1}{1 - w_{m - 1}}$.}\commentRS{I agree}
	...
	as $\frac{n-1}{n}>w_{m-1}$ \commentOC{I’d write: as $n > \frac{1}{1 - w_{m - 1}}$ (and specifying, “equivalently, $n - 1 > n w_{m - 1}$” if you think it is appropriate).}\commentRS{I agree}, we have $s^{w}(a_{1}; P)>s^{w}(a_{m};$ $P)$,
...
\ac{SCR} $f_{q}$ defined for each $P\in \linors^N$ as $f_{q}(P)=\{x\in A \suchthat n_{\rho _{q}(P)}(x,P)\geq n_{\rho _{q}(P)}(y,P)$ $\forall y\in A\}.$ \commentOC{Shouldn’t we also say BK compromise in the introduction, then?}\commentRS{I agree}
...
Indeed, this section endeavors to show that a mild supplementary restriction removes the positive results concerning the only two rules that we found to be compatible with any of our compromise concepts. \commentOC{Added this sentence to summarize the results of the section. To be phrased better?}
...
The condition is more convincing for larger values of $m$ and $n$ \commentOC{“convincing” might not be the best term here; and I think we are justified in claiming something stronger to justify our definition. I propose: “This condition appears to impose a very reasonable requirement on spread measures for large values of $m$ and $n$”}. Asking for $\sigma(1,3,2,2)$ to be smaller than $\sigma(2,1,0,0)$ is demanding while asking for $\sigma(5,7,6,6,6,6,6)$ to be smaller than $\sigma(6,5,4,3,2,1,0)$ reflects a mild assumption. In any case, as we state below, several well-known spread measures of the literature (see \citet{Allison1978} for a comprehensive account) satisfy \cref{def:conditionC}. Letting $\bar{l} = \sum_{i=1}^{n} l_i / n$ denote the arithmetic mean of the values of $l = (l_1, …, l_n)$, we consider the following measures:
...
\label{prop:spreadMeas}
	...
	\commentOC{TODO check checkProposition}
\end{remark}
...
\commentOC{Beatrice, please check the bibliography again: entries with dois should have URLs removed; and dois should be added where possible (e.g., Barberà).}
