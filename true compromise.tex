...
\end{proposition}
\commentOC{Didn’t we have some similar result whose proof was easier? Let’s try to simplify it, under condition that the other results still follow.} //TOFIND
\begin{proof}
...
\begin{proof}
Take any $m$ $\geq 3$ and any $w$ with $w_{m-k}<1$ for some $k\in \intvl{1, m-2}$. Define $n = \max\set*{\ceil*{\frac{m-1}{k}}, \floor*{\frac{1}{1-w_{m-k}}}+1}$. \Cref{whenPCCfails} then applies, because $n ≥ \frac{m-1}{k}$, and because $n > \frac{1}{1-w_{m-k}}$ which in turn implies $1-w_{m-k} > \frac{1}{n}$ and $1-\frac{1}{n} > w_{m-k}$.
%Remzi proof: First, observe the existence of $n_{1}\geq 3$ satisfying $k\cdot n_{1}\geq m-1$. Next, note that $\frac{n-1}{n}>w_{m-k}$ can be ensured with a sufficiently large $n$ if and only if $w_{m-k}<1$. So pick some $n_{2}\geq 3$ with $\frac{n_{2}-1}{n_{2}}>w_{m-k}$ . Taking $n=\max \left\{ n_{1},\text{ }n_{2}\right\} $, through Proposition \ref{whenPCCfails}%, we see that $f^{w}$ fails PCC.
\end{proof}
