...
\begin{proof}	
	...
	First, from \cref{prop:muSigmaLast}, $\set{a_m} \cap \paretopt(P) = \emptyset$, hence $\set{a_m} \cap {\mustar[\sigma][P]} = \emptyset$. 
...
\begin{proof}
	...
	\commentOC{Here (and in the next section) we use $\min$ in (what I believe to be) the most usual sense, whereas elsewhere we use $\min$ instead of $\argmin$. Should we change all these $\min$ to $\argmin$ (my favorite option)? Or should we say that we abuse notation?}\commentRS{Let's go for your favorite option}\commentBN{With "all these" you mean only the latter ones, right?}
	...
	By definition of $\bar\sigma$, $\bar\sigma(x) ≤ 1$, hence, $x \in \mustar[\bar\sigma]$ \commentOC{Why “$\bar\sigma(x) ≤ 1$, hence”? I propose to write: “As $k = 1$, $x \in \mustar[\bar\sigma]$.”}.\commentRS{I agree}\commentBN{The argument is that since $\bar\sigma(x)$ is by definition $≤ 1$ and the minimum value of $\bar\sigma \circ \lambda_P$ is $1$, then $x \in \mustar[\bar\sigma]$. I'm not sure that just saying "as k=1" conveys it.}
...
\ac{SCR} $f_{q}$ defined for each $P\in \linors^N$ as $f_{q}(P)=\{x\in A \suchthat n_{\rho _{q}(P)}(x,P)\geq n_{\rho _{q}(P)}(y,P)$ $\forall y\in A\}.$ \commentOC{Shouldn’t we also say BK compromise in the introduction, then?}\commentRS{I agree}\commentBN{You mean without hyphen?}
