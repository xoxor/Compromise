...
\commentOC{I’d like to suggest to integrate the footnotes in the text.}}
...
\commentOC{Perhaps we should move this § above, to present together the two rules that are defined for all $m$, then the two rules that are defined for $m$ odd; and to match the order used in the proof. Unless we find it more suitable to follow the chronological order.}
...
\end{definition}
For $m=4$ the condition requires $\sigma(2,1)$ to be smaller than $\sigma(0,2)$ which is reasonable when looking for a compromise. When the value of $m$ is larger, the condition appears even more convincing. As $m$ grows, the distance between $0$ and $\floor{\frac{m}{2}}$ grows, while the distance between $\ceil{\frac{m}{2}}$ and $\floor{\frac{m}{2}} - 1$ remains constant ($1$ if $m$ is even, $2$ otherwise). Requiring, for example, the spread of $(15,14)$ to be smaller than the spread of $(0,15)$ is very reasonable.
...
\begin{proof}
	...
	Consider the following profile P:
		\begin{center}
			...
			$.
		\end{center}
	\commentOC{This display may be misleading, because this alignment of alternatives is only correct for $m$ even (that is, when $\alpha = \beta + 1$). I suggest to not display $P$, and write instead: Consider the profile $P$ where voter $1$ has the preference order $x > a_1 > … > a_\alpha > y > b_1 > … > b_\beta$ and voter $2$ has the preference order $b_1 > … > b_\beta > y > x > a_1 > … > a_\alpha$.}
	...
	For PV, let $v_{i_1} ≥ v_{i_2}$ (thus $v_{i_1} ≥ \ceil{\frac{m-1}{2}}$ and $v_{i_2} ≤ \floor{\frac{m - 1}{2}}$, or equivalently, $v_{i_1} ≥ \floor{\frac{m}{2}} = \beta + 1$ and $v_{i_2} ≤ \ceil{\frac{m - 2}{2}} = \alpha$) \commentOC{Modified this parenthesis.}, and consider the profile $P$. Observe that the first voter vetoes at least $y$ and every $b_j$ ($1 ≤ j ≤ \beta$) while no voter vetoes $x$. As $x$ Pareto-dominates every $a_j$ ($1 ≤ j ≤ \alpha$), PV picks only $x$ at $P$. When $v_{i_2} ≥ v_{i_1}$, a similar reasoning yields that PV picks only $x$ at $P'$. \commentBN{I think I got it, $\forall \ v_1, v_2$ we want to find a profile for which it fails. Is it worth to add few words to clarify this? }
...
\end{proof}
 ...
 \end{definition}
Please note that such an alternative always exists.\commentOC{Under condition that $m$ is odd and “the first half” is defined as the bigger half.}

\begin{proposition} FB, PV (when the veto powers are equally distributed), SL and VR all satisfy \textit{First Half}.
\end{proposition}
\begin{proof}
	For FB, it is straightforward to see that, even if the two voters have opposite preferences, when descending level to reach unanimity we reach at most rank $\ceil{\frac{m}{2}}$ \commentOC{I’d say $\ceil{\frac{m + 1}{2}}$, which is equivalent when $m$ is odd but also works when $m$ is even}.
...
Now, suppose, under a given sigma, we pick the among the PV alternatIves the one with the smallest loss spread. For example, at the profile 
\begin{center}
	$
	\begin{array}{cccccc}
...
We define an ex-post compromise as an outcome where individuals give up as equally as possible \commentOC{(suggestion)} from their ideal points. With three or more individuals, several interesting \acp{SCR} of the literature (that can be interpreted as voting rules), namely Condorcet extensions, scoring rules (except antiplurality) and BK-compromises (except fallback bargaining where threshold is unanimity) fail to pick ex-post compromises, under any reasonable meaning attributed to “giving up equally”. This failure is valid whether Pareto optimality is adopted or not while the positive results for antiplurality and fallback bargaining quickly vanish when a mild condition is imposed over the definition of “giving up equally”. Thus, we can conclude that all these \acp{SCR} are not compatible with being ex-post compromises. Even BK-compromises are procedural or ex-ante compromises. They impose a willingness to compromise over individuals but they may eventually pick an outcome so that some individuals do not effectively compromise at all.
These findings are rather expected, as voting rules are typically unconcerned about envy-freeness among voters while this is the underlying notion of an ex-post compromise. 
\commentOC{Can we find some way of saying essentially the same thing without using the envy-freeness terminology, which I find misleading?}
On the other hand, this concern arguably should not weight much in the context of two individuals where the collective choice problem is interpreted as a bargaining problem that requires mutual consent. But all well-known \acp{SCR} of the literature, namely, fallback bargaining, Pareto and veto rules, short listing and veto rank, fail to pick ex-post compromises in this context. 

\commentOC{Beatrice, can you please have a look at the refs? Each of them should have a DOI if possible, and should have an URL iff they do not have a DOI.}
...
\begin{proof} for $\sigma_{mad}$. \\
	Let us define $f_l(i)= \sum_{j}|l_i-l_j|$, thus $\sigma_{mad}(l) = frac{1}{n^2}\sum_{i} f_l(i)$
	
	Consider the two vectors $l_1=(m-3, m-1, m-2, \dots, m-2)$ (where $n-2$ terms are equal to $m-2$) and $l_2=\sigma(m-2, m-3, \dots, 1, 0, \dots, 0)$ (where $m-2$ terms going from $m-2$ to $1$, and $m-n+2$ terms equal to $0$). 
