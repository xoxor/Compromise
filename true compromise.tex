...
\commentBN{Read papers \citep{Sprumont1993} and \citep{Barbera2020} and cite them.}
Collective choice environments with two-individuals are usually viewed as arbitration or bargaining problems.There are analyses of these problems, such as \citep{Kibris2007}, where the model admits an explicit disagreement outcome. We remain in the classical collective choice environment with no explicit disagreement alternative.
...
\begin{remark}
Note that PV fails PCC when $m=6$.
\end{remark}
\commentBN{ P\\
 x   a1  a2   y   b1  b2 \\
b1   b2  y    x   a1  a2 
...
\commentOC{PV may satisfy PCC for $\sigma(l_1, l_2) = \abs{l_1 - l_2}$ for $m\leq5$}
