...
Note that both vectors consists of $n$ values, and that the terms $m-2, m-3, \dots,1$, of the second vector, represent the first $m-1$ components of that vector.
...
In the years many measures of inequality have been proposed \citep{Jackman1974, Rubinson1977, Martin1971, Ray1973, Schutz1951, Theil1967} \commentBN{Do I remove the references as well?} but perhaps the most commonly used is the Gini coefficient, G, which is defined as the ratio of the area between the Lorenz curve and the diagonal (line of equality), over the total area under the line of equality.
\[\sigma_{G}(r)= \frac{\sum_{i=1}^{n}\sum_{j=1}^{n}|r_i-r_j|}{2 \cdot n \cdot \sum_{i=1}^{n} r_i}.\]

Some other measures of dispersion are:
\begin{itemize}
	\item the standard deviation $\sigma_{sd}(r)= \sqrt{\frac{\sum_{i=1}^{n}(r_i-\bar{r})^2}{n}}$
	\item the mean absolute difference $\sigma_{mad}(r)= \frac{1}{n^2} \sum_{i=1}^{n}\sum_{j=1}^{n}|r_i-r_j|$
	\item the average absolute deviation $\sigma_{ad}(r)= \frac{\sum_{i=1}^{n}|r_i-\bar{r}|}{n}$
\end{itemize}
\commentBN{Maybe we can remove the formulas from here since we show them later. Or we can move all this discussion about known spread measures in the next section.}  
In what follows we investigate which of the spread measures just presented satisfy the condition of \cref{def:conditionC}. The proofs for the following theorems can be found in \cref{apdx:proofSM}.
\subsubsection{Spread Measures and Condition C}
\label{sec:spreadMeasures}
...
The \textbf{Average Absolute Deviation} is a measure that consider the differences between the values of the vector and their arithmetic mean $\bar{r}= \frac{\sum_{i=1}^{n} r_i}{n}$.
\[\sigma_{ad}(r)= \frac{\sum_{i=1}^{n}|r_i-\bar{r}|}{n}\]

\begin{theorem}
	\label{th:ad}
	$\sigma_{ad}$ satisfies condition C $\forall m \geq 4 $ and $ n\geq 4$.
\end{theorem}

The \textbf{Standard Deviation} is a measure that indicates the dispersion of a set of data relatively to its mean.
\[\sigma_{sd}(r)= \sqrt{\frac{\sum_{i=1}^{n}(r_i-\bar{r})^2}{n}}\]

\begin{theorem}
	\label{th:sd}
	$\sigma_{sd}$ satisfies condition C $\forall m \geq 4 $ and $n\geq 4$.
\end{theorem}

The \textbf{Gini Coefficient} is a measure that aim to represent the degree of inequality in a distribution. Alternatively to the definition we gave above, based on the Lorenz curve, we can define it as half of the relative mean absolute difference. Where the relative mean absolute difference is the mean absolute difference divided by the average.
\[\sigma_{G}(r)= \frac{\sigma_{mad}}{2\cdot\bar{r}}= \frac{\sum_{i=1}^{n}\sum_{j=1}^{n}|r_i-r_j|}{2 \cdot n \cdot \sum_{i=1}^{n} r_i}\]
\begin{theorem}
	\label{th:gini}
	$\sigma_{G}$ satisfies condition C $\forall m \geq 4$ and $n\geq 4$.
\end{theorem}
\commentBN{To check again. I believe this is true but the computation doesn't give the same result.}
...
\label{apdx:proofSM}
In what follows are showed the proofs for the Theorems presented in \cref{sec:spreadMeasures}.
\begin{proof} for \cref{th:mad}. \\
