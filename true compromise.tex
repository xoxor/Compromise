...
\commentOC{I suggest this alternative wording, which seems more direct to me.} \commentBN{I still prefer the former one but I don't mind this either if you think it would be easier for the reader.} \commentRS{Both are fine for me. Let's toss a coin!}
\begin{proof}
...
%	We then have that $x \in \mustar[\bar\sigma]$ as by definition of $\bar\sigma$, $\bar\sigma(x) ≤ 1$.
	...
	If $p = 0$, take any $x \in \mustar[\bar\sigma]$. As $\bar\sigma (\lambda _{P}(x))=0$, we have, $\forall i, j \in N$: $\lambda_i^P(x) = \lambda_j^P(x)$, hence, $\forall i, j \in N$: $r_{\succ_i}(x) = r_{\succ_j}(x)$. 
...
\commentOC{Indeed, if we relied on such a specific ordering of alternatives, we would need to spell it out. But I don’t think this is the case. The suggested profile is excluded by $n ≥ m - 1$. More generally, $P$ guarantees, looking only at the first and last two columns, that only $a_m$ is “same-ranked”, as all alternatives in $a_1, …, a_{m - 1}$ appear exactly once in the last rank.}
 \commentRS{I agree with what Olivier says. I think that column can and better be removed.}
...
\commentBN{Is it clear that the first dots mean $m-2,m-2,m-2,m-2$ while the second ones $m-4,m-5,m-6...$? Also I think it should be $\sigma(m-2, m-3, \dots, 1, 0, \dots, 0)$?}
 \commentRS{I don't think there is anything unclear but we can discuss this on Monday. In any case, I don't think there are dots between the zeros.}
...
\begin{theorem}
	$\sigma_{mad}$ satisfies condition C for $\forall m\geq4$ and $n>\frac{2 \cdot m^3 - 9\cdot m^2 + 13\cdot m - 18}{3 (m^2 - 3 \cdot m - 2)}$.
\end{theorem}
\begin{proof} 
	...
	which is true for all $m> \frac{1}{2}(3+\sqrt{17})\approx 3.56$ and $n>\frac{2 \cdot m^3 - 9\cdot m^2 + 13\cdot m - 18}{3 (m^2 - 3 \cdot m - 2)}$.
	\commentBN{The inequality works also for other values of m and n lower than $3$.}
\end{proof}


The \textbf{Average Absolute Deviation} \commentBN{maybe it is more known as Mean Absolute Deviation?} is a measure that consider the differences between the values of the vector and their arithmetic mean $\bar{r}$.
