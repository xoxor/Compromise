...
From now on, we will ensure the Pareto optimality of cardinal compromises by defining them as $C^{\sigma }(u)=\left\{ x\in PO(u):\sigma (\lambda ^{u}(x))\leq \sigma (\lambda ^{u}(y)),  \forall y\in A\right\}$. \commentOC{Damned, my communist propaganda failed again.} \commentRS{Not necessarily. This is an inception for future resolution.}
